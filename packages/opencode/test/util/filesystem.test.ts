...
describe("filesystem", () => {
  ...
  describe("findUp()", () => {
    ...
    test("rootFirst preserves json then jsonc order per directory", async () => {
      ...
      await fs.writeFile(path.join(tmp.path, "flashcode.json"), "{}", "utf-8")
      await fs.writeFile(path.join(tmp.path, "flashcode.jsonc"), "{}", "utf-8")
      await fs.writeFile(path.join(project, "flashcode.json"), "{}", "utf-8")
      await fs.writeFile(path.join(project, "flashcode.jsonc"), "{}", "utf-8")

      const result = await Filesystem.findUp(["flashcode.json", "flashcode.jsonc"], nested, tmp.path, {
        rootFirst: true,
      })

      expect(result).toEqual([
        path.join(tmp.path, "flashcode.json"),
        path.join(tmp.path, "flashcode.jsonc"),
        path.join(project, "flashcode.json"),
        path.join(project, "flashcode.jsonc"),
      ])
    })
  })
