...
afterEach(async () => {
  ...
  if (original.envPassword === undefined) delete process.env.FLASHCODE_SERVER_PASSWORD
  else process.env.FLASHCODE_SERVER_PASSWORD = original.envPassword
  if (original.envUsername === undefined) delete process.env.FLASHCODE_SERVER_USERNAME
  else process.env.FLASHCODE_SERVER_USERNAME = original.envUsername
  await disposeAllInstances()
  await resetDatabase()
})

async function startListener() {
  Flag.FLASHCODE_SERVER_PASSWORD = auth.password
  Flag.FLASHCODE_SERVER_USERNAME = auth.username
  process.env.FLASHCODE_SERVER_PASSWORD = auth.password
  process.env.FLASHCODE_SERVER_USERNAME = auth.username
  return Server.listen({ hostname: "127.0.0.1", port: 0 })
}

async function startNoAuthListener() {
  Flag.FLASHCODE_SERVER_PASSWORD = undefined
  Flag.FLASHCODE_SERVER_USERNAME = auth.username
  delete process.env.FLASHCODE_SERVER_PASSWORD
  process.env.FLASHCODE_SERVER_USERNAME = auth.username
  return Server.listen({ hostname: "127.0.0.1", port: 0 })
}
