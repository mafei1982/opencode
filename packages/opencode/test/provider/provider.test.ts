...
test("local_tcp provider is injected from env", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      await Bun.write(
        path.join(dir, "flashcode.json"),
  ...
  try {
    ...
    delete process.env.LLM_N_CTX
    delete process.env.LLM_MAX_CONCURRENCY
    delete process.env.LLM_PARALLEL_N
  }
})

test("local_tcp mode auto-restricts enabled providers", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      await Bun.write(
        path.join(dir, "flashcode.json"),
...
test("plugin config providers persist after instance dispose", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const configDir = path.join(dir, ".flashcode")
...
test("plugin config enabled and disabled providers are honored", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const root = path.join(dir, ".flashcode", "plugin")
      await mkdir(root, { recursive: true })
      await Bun.write(
