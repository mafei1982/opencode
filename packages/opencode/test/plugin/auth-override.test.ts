            ...
            get: () =>
              Effect.succeed({
                ...
                plugin_origins: plugins.map((plugin) => ({
                  spec: plugin,
                  source: path.join(directory, "flashcode.json"),
...
describe("plugin.auth-override", () => {
  test("user plugin overrides built-in github-copilot auth", async () => {
    await using tmp = await tmpdir({
      init: async (dir) => {
        const pluginDir = path.join(dir, ".flashcode", "plugin")
    ...
    const plugin = pathToFileURL(path.join(tmp.path, ".flashcode", "plugin", "custom-copilot-auth.ts")).href
