...
async function writeManagedSettings(settings: object, filename = "flashcode.json") {
  await fs.mkdir(managedConfigDir, { recursive: true })
  await Filesystem.write(path.join(managedConfigDir, filename), JSON.stringify(settings))
}

async function writeConfig(dir: string, config: object, name = "flashcode.json") {
...
test("creates global jsonc config with schema when no global configs exist", async () => {
  ...
  try {
    ...
    const content = await Filesystem.readText(path.join(tmp.path, "flashcode.jsonc"))
...
test("does not create global config when FLASHCODE_CONFIG_DIR is set", async () => {
  await using tmp = await tmpdir()
  await using custom = await tmpdir()
  const prevConfig = Global.Path.config
  const prevEnv = process.env.FLASHCODE_CONFIG_DIR
  ;(Global.Path as { config: string }).config = tmp.path
  process.env.FLASHCODE_CONFIG_DIR = custom.path
  await clear(true)
  ...
  try {
    ...
    expect(await Filesystem.exists(path.join(tmp.path, "flashcode.jsonc"))).toBe(false)
  } finally {
    ;(Global.Path as { config: string }).config = prevConfig
    if (prevEnv === undefined) delete process.env.FLASHCODE_CONFIG_DIR
    else process.env.FLASHCODE_CONFIG_DIR = prevEnv
    await clear(true)
  }
})
...
test("updates global config and omits empty shell key in json", async () => {
  ...
  try {
    ...
    const writtenConfig = await Filesystem.readJson<{ shell?: string }>(path.join(tmp.path, "flashcode.json"))
...
test("updates global config and omits empty shell key in jsonc", async () => {
  ...
  try {
    ...
    const file = path.join(tmp.path, "flashcode.jsonc")
  ...
  try {
    ...
    await WithInstance.provide({
      ...
      fn: async () => {
        ...
        const content = await Filesystem.readText(path.join(tmp.path, "flashcode.json"))
...
test("throws error for invalid JSON", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      await Filesystem.write(path.join(dir, "flashcode.json"), "{ invalid json }")
    },
  })
  await provideTestInstance({
    directory: tmp.path,
...
test("loads config from .flashcode directory", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const opencodeDir = path.join(dir, ".flashcode")
...
test("agent markdown permission config preserves user key order", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const agentDir = path.join(dir, ".flashcode", "agent")
      await fs.mkdir(agentDir, { recursive: true })
...
test("loads agents from .flashcode/agents (plural)", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const opencodeDir = path.join(dir, ".flashcode")
      await fs.mkdir(opencodeDir, { recursive: true })
...
test("loads commands from .flashcode/command (singular)", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const opencodeDir = path.join(dir, ".flashcode")
      await fs.mkdir(opencodeDir, { recursive: true })
...
test("loads commands from .flashcode/commands (plural)", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const opencodeDir = path.join(dir, ".flashcode")
      await fs.mkdir(opencodeDir, { recursive: true })
...
test("does not try to install dependencies in read-only FLASHCODE_CONFIG_DIR", async () => {
  if (process.platform === "win32") return
  ...
  const prev = process.env.FLASHCODE_CONFIG_DIR
  process.env.FLASHCODE_CONFIG_DIR = tmp.extra
  ...
  try {
    ...
    if (prev === undefined) delete process.env.FLASHCODE_CONFIG_DIR
    else process.env.FLASHCODE_CONFIG_DIR = prev
  }
})

test("installs dependencies in writable FLASHCODE_CONFIG_DIR", async () => {
  await using tmp = await tmpdir<string>({
    init: async (dir) => {
...
test("merges plugin arrays from global and local configs", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      // Create a nested project structure with local .flashcode config
      const projectDir = path.join(dir, "project")
      const opencodeDir = path.join(projectDir, ".flashcode")
      await fs.mkdir(opencodeDir, { recursive: true })
      ...
      // Local .flashcode config with different plugins
      await Filesystem.write(
        path.join(opencodeDir, "flashcode.json"),
        JSON.stringify({
          $schema: "https://opencode.ai/config.json",
...
test("deduplicates duplicate plugins from global and local configs", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      ...
      // Local .flashcode config with some overlapping plugins
      await Filesystem.write(
        path.join(opencodeDir, "flashcode.json"),
        JSON.stringify({
          $schema: "https://opencode.ai/config.json",
...
test("keeps plugin origins aligned with merged plugin list", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      ...
      const local = path.join(project, ".flashcode")
      ...
      await Filesystem.write(
        path.join(local, "flashcode.json"),
        JSON.stringify({
          $schema: "https://opencode.ai/config.json",
...
test("local .flashcode config can override MCP from project config", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      ...
      // Local .flashcode directory config enables it
      const opencodeDir = path.join(dir, ".flashcode")
      await fs.mkdir(opencodeDir, { recursive: true })
      await Filesystem.write(
        path.join(opencodeDir, "flashcode.json"),
    ...
    if (urlStr.includes(".well-known/opencode")) {
      ...
      return Promise.resolve(
        new Response(
          JSON.stringify({
            remote_config: {
              url: "https://config.example.com/flashcode.json",
              headers: {
      ...
      () =>
        Config.Service.use((svc) =>
          Effect.gen(function* () {
            ...
            expect(remoteFetchedUrl).toBe("https://config.example.com/flashcode.json")
...
describe("resolvePluginSpec", () => {
  test("keeps package specs unchanged", async () => {
    await using tmp = await tmpdir()
    const file = path.join(tmp.path, "flashcode.json")
...
describe("deduplicatePluginOrigins", () => {
  ...
  test("keeps path plugins separate from package plugins", () => {
    const plugins = ["oh-my-opencode@2.4.3", "file:///project/.flashcode/plugin/oh-my-opencode.js"]
  ...
  test("deduplicates direct path plugins by exact spec", () => {
    const plugins = ["file:///project/.flashcode/plugin/demo.ts", "file:///project/.flashcode/plugin/demo.ts"]

    const result = dedupe(plugins)

    expect(result).toEqual(["file:///project/.flashcode/plugin/demo.ts"])
  })
...
describe("FLASHCODE_DISABLE_PROJECT_CONFIG", () => {
  test("skips project config files when flag is set", async () => {
    const originalEnv = process.env["FLASHCODE_DISABLE_PROJECT_CONFIG"]
    process.env["FLASHCODE_DISABLE_PROJECT_CONFIG"] = "true"
    ...
    try {
      ...
      if (originalEnv === undefined) {
        delete process.env["FLASHCODE_DISABLE_PROJECT_CONFIG"]
      } else {
        process.env["FLASHCODE_DISABLE_PROJECT_CONFIG"] = originalEnv
      }
    }
  })

  test("skips project .flashcode/ directories when flag is set", async () => {
    const originalEnv = process.env["FLASHCODE_DISABLE_PROJECT_CONFIG"]
    process.env["FLASHCODE_DISABLE_PROJECT_CONFIG"] = "true"
    ...
    try {
      await using tmp = await tmpdir({
        init: async (dir) => {
          // Create a .flashcode directory with a command
          const opencodeDir = path.join(dir, ".flashcode", "command")
          await fs.mkdir(opencodeDir, { recursive: true })
      ...
      await WithInstance.provide({
        ...
        fn: async () => {
          ...
          // Project .flashcode should NOT be in directories list
  ...
  test("skips relative instructions with warning when flag is set but no config dir", async () => {
    const originalDisable = process.env["FLASHCODE_DISABLE_PROJECT_CONFIG"]
    const originalConfigDir = process.env["FLASHCODE_CONFIG_DIR"]

    try {
      // Ensure no config dir is set
      delete process.env["FLASHCODE_CONFIG_DIR"]
      process.env["FLASHCODE_DISABLE_PROJECT_CONFIG"] = "true"
      ...
      if (originalDisable === undefined) {
        ...
        process.env["FLASHCODE_DISABLE_PROJECT_CONFIG"] = originalDisable
      }
      if (originalConfigDir === undefined) {
        delete process.env["FLASHCODE_CONFIG_DIR"]
      } else {
        process.env["FLASHCODE_CONFIG_DIR"] = originalConfigDir
      }
    }
  })

  test("FLASHCODE_CONFIG_DIR still works when flag is set", async () => {
    const originalDisable = process.env["FLASHCODE_DISABLE_PROJECT_CONFIG"]
    const originalConfigDir = process.env["FLASHCODE_CONFIG_DIR"]
    ...
    try {
      ...
      process.env["FLASHCODE_DISABLE_PROJECT_CONFIG"] = "true"
      process.env["FLASHCODE_CONFIG_DIR"] = configDirTmp.path

      await WithInstance.provide({
        directory: projectTmp.path,
        fn: async () => {
          const config = await load()
          // Should load from FLASHCODE_CONFIG_DIR, not project
...
describe("FLASHCODE_CONFIG_CONTENT token substitution", () => {
  test("substitutes {env:} tokens in FLASHCODE_CONFIG_CONTENT", async () => {
    const originalEnv = process.env["FLASHCODE_CONFIG_CONTENT"]
    const originalTestVar = process.env["TEST_CONFIG_VAR"]
    process.env["TEST_CONFIG_VAR"] = "test_api_key_12345"
    process.env["FLASHCODE_CONFIG_CONTENT"] = JSON.stringify({
      $schema: "https://opencode.ai/config.json",
    ...
    try {
      ...
      if (originalEnv !== undefined) {
        process.env["FLASHCODE_CONFIG_CONTENT"] = originalEnv
      } else {
        delete process.env["FLASHCODE_CONFIG_CONTENT"]
      }
  ...
  test("substitutes {file:} tokens in FLASHCODE_CONFIG_CONTENT", async () => {
    const originalEnv = process.env["FLASHCODE_CONFIG_CONTENT"]
    ...
    ConfigParse.jsonc(
      await ConfigManaged.parseManagedPlist(
        JSON.stringify({
          ...
          PayloadIdentifier: "ai.flashcode.managed.test",
          PayloadType: "ai.flashcode.managed",
