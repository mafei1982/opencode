...
afterEach(async () => {
  ...
  await fs.rm(path.join(Global.Path.config, "flashcode.json"), { force: true }).catch(() => {})
  await fs.rm(path.join(Global.Path.config, "flashcode.jsonc"), { force: true }).catch(() => {})
...
test("keeps server and tui plugin merge semantics aligned", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const local = path.join(dir, ".flashcode")
      await fs.mkdir(local, { recursive: true })

      await Bun.write(
        path.join(Global.Path.config, "flashcode.json"),
        JSON.stringify(
          {
...
test("loads tui config with the same precedence order as server config paths", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      ...
      await fs.mkdir(path.join(dir, ".flashcode"), { recursive: true })
      await Bun.write(
        path.join(dir, ".flashcode", "tui.json"),
...
test("migrates tui-specific keys from flashcode.json when tui.json does not exist", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      await Bun.write(
  ...
  const server = JSON.parse(await Filesystem.readText(path.join(tmp.path, "flashcode.json")))
  expect(server.theme).toBeUndefined()
  expect(server.keybinds).toBeUndefined()
  expect(server.tui).toBeUndefined()
  expect(await Filesystem.exists(path.join(tmp.path, "flashcode.json.tui-migration.bak"))).toBe(true)
...
test("skips migration when flashcode.jsonc is syntactically invalid", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      await Bun.write(
  ...
  expect(await Filesystem.exists(path.join(tmp.path, "flashcode.jsonc.tui-migration.bak"))).toBe(false)
  const source = await Filesystem.readText(path.join(tmp.path, "flashcode.jsonc"))
...
test("skips migration when tui.json already exists", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      await Bun.write(path.join(dir, "flashcode.json"), JSON.stringify({ theme: "legacy" }, null, 2))
  ...
  expect(await Filesystem.exists(path.join(tmp.path, "flashcode.json.tui-migration.bak"))).toBe(false)
})

test("continues loading tui config when legacy source cannot be stripped", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      await Bun.write(path.join(dir, "flashcode.json"), JSON.stringify({ theme: "readonly-theme" }, null, 2))
    },
  })

  const source = path.join(tmp.path, "flashcode.json")
...
test("migration backup preserves JSONC comments", async () => {
  ...
  const backup = await Filesystem.readText(path.join(tmp.path, "flashcode.jsonc.tui-migration.bak"))
...
test("migrates legacy tui keys across multiple flashcode.json levels", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const nested = path.join(dir, "apps", "client")
      await fs.mkdir(nested, { recursive: true })
      await Bun.write(path.join(dir, "flashcode.json"), JSON.stringify({ theme: "root-theme" }, null, 2))
      await Bun.write(path.join(nested, "flashcode.json"), JSON.stringify({ theme: "nested-theme" }, null, 2))
    },
  })
...
test("project config takes precedence over FLASHCODE_TUI_CONFIG (matches FLASHCODE_CONFIG)", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      ...
      process.env.FLASHCODE_TUI_CONFIG = custom
    },
  })
...
test("FLASHCODE_TUI_CONFIG provides settings when no project config exists", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const custom = path.join(dir, "custom-tui.json")
...
test("does not derive tui path from FLASHCODE_CONFIG", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const customDir = path.join(dir, "custom")
      await fs.mkdir(customDir, { recursive: true })
      await Bun.write(path.join(customDir, "flashcode.json"), JSON.stringify({ model: "test/model" }))
      await Bun.write(path.join(customDir, "tui.json"), JSON.stringify({ theme: "should-not-load" }))
      process.env.FLASHCODE_CONFIG = path.join(customDir, "flashcode.json")
    },
  })
  const config = await getTuiConfig(tmp.path)
  expect(config.theme).toBeUndefined()
...
test("loads .flashcode/tui.json", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      await fs.mkdir(path.join(dir, ".flashcode"), { recursive: true })
      await Bun.write(path.join(dir, ".flashcode", "tui.json"), JSON.stringify({ diff_style: "stacked" }, null, 2))
    },
  })
  const config = await getTuiConfig(tmp.path)
  expect(config.diff_style).toBe("stacked")
...
test("silently skips malformed tui.json — load failures degrade to {}", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      ...
      await Bun.write(path.join(dir, ".flashcode", "tui.json"), JSON.stringify({ theme: "fallback" }))
    },
  })

  const config = await getTuiConfig(tmp.path)
  // Project tui.json is malformed → silently skipped (logs a warning)
  // .flashcode/tui.json (lower precedence in this path) still loads
...
test("silently skips non-ENOENT read failures (e.g. tui.json is a directory) — fallback layer still loads", async () => {
  ...
  const config = await getTuiConfig(tmp.path)
  // Did NOT crash; .flashcode/tui.json (lower precedence) still loads.
  expect(config.theme).toBe("fallback")
})
