...
function custom(dep: CustomDep): Record<string, CustomLoader> {
  return {
    ...
    local_tcp: Effect.fnUntraced(function* () {
      const env = { ...process.env, ...pickBy(yield* dep.env(), (value) => value !== undefined) }
      if ((env.LLM_PROVIDER ?? "").toLowerCase() !== "local_tcp") return { autoload: false }
      const integer = (value: string | undefined) => (value ? Number.parseInt(value, 10) : undefined)
      const decimal = (value: string | undefined) => (value ? Number.parseFloat(value) : undefined)
      const boolean = (value: string | undefined) => (value ? value.toLowerCase() === "true" : undefined)
      return {
        autoload: true,
        options: {
          modelPath: env.LLM_MODEL_PATH,
          nCtx: integer(env.LLM_N_CTX),
          nGpuLayers: integer(env.LLM_N_GPU_LAYERS),
          nGpuLayersDraft: env.LLM_N_GPU_LAYERS_DRAFT,
          splitMode: env.LLM_SPLIT_MODE,
          devices: env.LLM_CUDA_DEVICES,
          batchSize: integer(env.LLM_BATCH_SIZE),
          threads: integer(env.LLM_THREADS),
          maxThreads: integer(env.LLM_MAX_THREADS),
          sequences: integer(env.LLM_PARALLEL_N ?? env.LLM_SEQUENCES ?? env.LLM_MAX_CONCURRENCY),
          kvUnified: boolean(env.LLM_KV_UNIFIED),
          cacheRam: integer(env.LLM_CACHE_RAM),
          ctxCheckpoints: integer(env.LLM_CTX_CHECKPOINTS),
          checkpointMinStep: integer(env.LLM_CHECKPOINT_MIN_STEP),
          specType: env.LLM_SPEC_TYPE,
          specDraftNMax: integer(env.LLM_SPEC_DRAFT_N_MAX),
          specDraftNMin: integer(env.LLM_SPEC_DRAFT_N_MIN),
          specDraftPMin: decimal(env.LLM_SPEC_DRAFT_P_MIN),
          specDraftTypeK: env.LLM_SPEC_DRAFT_TYPE_K,
          specDraftTypeV: env.LLM_SPEC_DRAFT_TYPE_V,
          cacheTypeK: env.LLM_CACHE_TYPE_K,
          cacheTypeV: env.LLM_CACHE_TYPE_V,
          flashAttention: boolean(env.LLM_FLASH_ATTENTION),
          useMmap: boolean(env.LLM_USE_MMAP),
          useMlock: boolean(env.LLM_USE_MLOCK),
          disableThinking: boolean(env.LLM_DISABLE_THINKING),
          temperature: decimal(env.LLM_TEMPERATURE),
          topP: decimal(env.LLM_TOP_P),
          topK: integer(env.LLM_TOP_K),
          minP: decimal(env.LLM_MIN_P),
          repeatPenalty: decimal(env.LLM_REPEAT_PENALTY),
          repeatLastN: integer(env.LLM_REPEAT_LAST_N),
          inferenceTimeout: integer(env.LLM_INFERENCE_TIMEOUT),
          serverPath: env.LLM_TCP_SERVER_PATH,
          startupTimeout: integer(env.LLM_SERVER_START_TIMEOUT),
        },
      }
    }),
  }
  ...
  Effect.gen(function* () {
    ...
    const state = yield* InstanceState.make<State>(() =>
      Effect.gen(function* () {
        ...
        const envState = { ...process.env, ...pickBy(yield* env.all(), (value) => value !== undefined) }
        ...
        if (llmProvider === "local_tcp") {
          const providerID = ProviderV2.ID.make("local_tcp")
          const context = Number.parseInt(envState.LLM_N_CTX ?? "128000", 10)
          const local: Info = {
            id: providerID,
            ...
            models: {
              default: {
                ...
                providerID,
                name: `Local TCP Model (${envState.LLM_MODEL_PATH ?? "local-model"})`,
                family: "",
                api: { id: "default", url: "", npm: "@opencode/local-tcp" },
                status: "active",
                headers: {},
                options: {},
                cost: { input: 0, output: 0, cache: { read: 0, write: 0 } },
          ...
          catalog[providerID] = local
          database[providerID] = toPublicInfo(local)
        }
        ...
        const lockNonLocalProviders =
          llmProvider === "local_tcp" &&
          !["0", "false", "no", "off"].includes((envState.LLM_DISABLE_NON_LOCAL_PROVIDERS ?? "").trim().toLowerCase())
        const enabled = cfg.enabled_providers
          ? new Set(cfg.enabled_providers)
          : lockNonLocalProviders
            ? new Set([llmProvider])
            : null
