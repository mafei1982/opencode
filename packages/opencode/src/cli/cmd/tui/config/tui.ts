...
const loadState = Effect.fn("TuiConfig.loadState")(function* (ctx: { directory: string }) {
  ...
  const load = (text: string, configFilepath: string): Effect.Effect<Info> =>
    Effect.gen(function* () {
      ...
      // (mirroring the old flashcode.json shape) still get their settings applied.
  ...
  // Every config dir we may read from: global config dir, any `.flashcode`
  // folders between cwd and home, and FLASHCODE_CONFIG_DIR.
  const directories = yield* ConfigPaths.directories(ctx.directory)
  yield* Effect.promise(() => migrateTuiConfig({ directories, cwd: ctx.directory }))

  const projectFiles = Flag.FLASHCODE_DISABLE_PROJECT_CONFIG ? [] : yield* ConfigPaths.files("tui", ctx.directory)
  ...
  // 2. Explicit FLASHCODE_TUI_CONFIG override, if set.
  if (Flag.FLASHCODE_TUI_CONFIG) {
    const configFile = Flag.FLASHCODE_TUI_CONFIG
  ...
  // 4. `.flashcode` directories (and FLASHCODE_CONFIG_DIR) discovered while
  // walking up the tree. Also returned below so callers can install plugin
  // dependencies from each location.
  const dirs = unique(directories).filter((dir) => ConfigPaths.isConfigDirectory(dir) || dir === Flag.FLASHCODE_CONFIG_DIR)

  for (const dir of dirs) {
    if (!ConfigPaths.isConfigDirectory(dir) && dir !== Flag.FLASHCODE_CONFIG_DIR) continue
