...
export const McpAuthCommand = effectCmd({
  ...
  handler: Effect.fn("Cli.mcp.auth")(function* (args) {
    ...
    if (servers.length === 0) {
      ...
      prompts.log.info("Remote MCP servers support OAuth by default. Add a remote server in flashcode.json:")
      prompts.log.info(`
  "mcp": {
    "my-server": {
      "type": "remote",
...
async function resolveConfigPath(baseDir: string, global = false) {
  // Check for existing config files (prefer .jsonc over .json, check .flashcode/ subdirectory too)
  const candidates = [path.join(baseDir, "flashcode.json"), path.join(baseDir, "flashcode.jsonc")]

  if (!global) {
    candidates.push(path.join(baseDir, ".flashcode", "flashcode.json"), path.join(baseDir, ".flashcode", "flashcode.jsonc"))
  }
  ...
  // Default to flashcode.json if none exist
  return candidates[0]
}
