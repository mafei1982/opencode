...
const loadState = Effect.fn("TuiConfig.loadState")(function* (ctx: { directory: string }) {
  ...
  const load = (text: string, configFilepath: string): Effect.Effect<Info> =>
    Effect.gen(function* () {
      ...
      // (mirroring the old flashcode.json/opencode.json shape) still get their settings applied.
  ...
  // Every config dir we may read from: global config dir, any `.flashcode` or
  // `.opencode` folders between cwd and home, and the explicit config dir.
  ...
  // 4. Project config directories (and the explicit config dir) discovered while
  // walking up the tree. Also returned below so callers can install plugin
  // dependencies from each location.
  const dirs = unique(directories).filter(
    (dir) => ConfigPaths.isConfigDirectory(dir) || dir === Flag.OPENCODE_CONFIG_DIR,
  )

  for (const dir of dirs) {
    if (!ConfigPaths.isConfigDirectory(dir) && dir !== Flag.OPENCODE_CONFIG_DIR) continue
