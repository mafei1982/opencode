...
- If `engines.flashcode` is absent, no check is performed (backward compatible).
...
- For local scope, path is `<worktree>/.flashcode` only when VCS is git and `worktree !== "/"`; otherwise `<directory>/.flashcode`.
- Root-worktree fallback (`worktree === "/"` uses `<directory>/.flashcode`) is covered by regression tests.
...
When `plugin` entries exist in a writable `.flashcode` dir or `FLASHCODE_CONFIG_DIR`, OpenCode installs `@opencode-ai/plugin` into that dir and writes:
...
- Local plugins persist installed themes under the local `.flashcode/themes` area near the plugin config source.
...
- `--pure` / `FLASHCODE_PURE` skips external TUI plugins only.
...
- Local smoke plugin: `.flashcode/plugins/tui-smoke.tsx`
- Local vim plugin: `.flashcode/plugins/tui-vim.tsx`
- Local smoke config: `.flashcode/tui.json`
- Local smoke theme: `.flashcode/plugins/smoke-theme.json`
