...
export async function exportDebugLogs() {
  ...
  const output = join(app.getPath("downloads"), `flashcode-debug-${stamp()}.zip`)
  try {
...
function serverLogRoots() {
  ...
  return [
    ...new Set(
      ["flashcode", "ni-cic-code", "opencode"].flatMap((name) => [
        join(xdgData, name, "log"),
        join(app.getPath("userData"), name, "log"),
      ]),
    ),
  ]
}
