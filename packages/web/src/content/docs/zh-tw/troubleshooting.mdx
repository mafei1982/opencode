...
- **macOS/Linux**: `~/.config/flashcode/flashcode.jsonc`（或 `~/.config/flashcode/flashcode.json`）
- **macOS/Linux**（舊版安裝）: `~/.local/share/flashcode/flashcode.jsonc`
- **Windows**: 按 `WIN+R` 並貼上 `%USERPROFILE%\.config\opencode\flashcode.jsonc`
...
- **macOS**: Finder -> `Cmd+Shift+G` -> 貼上 `~/.cache/flashcode`
- **Linux**: 刪除 `~/.cache/flashcode`（或執行 `rm -rf ~/.cache/flashcode`）
...
如果你的 `flashcode.json(c)` 包含 `server` 部分，請暫時移除該部分並重新啟動桌面應用程式。
