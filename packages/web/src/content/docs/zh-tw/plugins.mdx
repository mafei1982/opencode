...
- `.flashcode/plugins/` - 專案級外掛
- `~/.config/flashcode/plugins/` - 全域外掛
...
**npm 外掛**在啟動時使用 Bun 自動安裝。套件及其相依套件會快取在 `~/.cache/flashcode/node_modules/` 中。
...
1. 全域設定 (`~/.config/flashcode/flashcode.json`)
2. 專案設定 (`flashcode.json`)
3. 全域外掛目錄 (`~/.config/flashcode/plugins/`)
4. 專案外掛目錄 (`.flashcode/plugins/`)
