...
:::note
You can disable automatic LSP server downloads by setting the `FLASHCODE_DISABLE_LSP_DOWNLOAD` environment variable to `true`.
:::
...
```json title="flashcode.json" {5-13}
{
  "$schema": "https://opencode.ai/config.json",
  "lsp": {
    "custom-lsp": {
