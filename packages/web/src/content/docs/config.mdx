...
2. **Global config** (`~/.config/flashcode/flashcode.json`) - user preferences
3. **Custom config** (`FLASHCODE_CONFIG` env var) - custom overrides
4. **Project config** (`flashcode.json` in project) - project-specific settings
5. **`.flashcode` directories** - agents, commands, plugins
6. **Inline config** (`FLASHCODE_CONFIG_CONTENT` env var) - runtime overrides
...
:::note
The `.flashcode` and `~/.config/flashcode` directories use **plural names** for subdirectories: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/`, and `themes/`. Singular names (e.g., `agent/`) are also supported for backwards compatibility.
:::
...
Place your global OpenCode config in `~/.config/flashcode/flashcode.json`. Use global config for user-wide server/runtime preferences like providers, models, and permissions.

For TUI-specific settings, use `~/.config/flashcode/tui.json`.
...
Add `flashcode.json` in your project root. Project config has the highest precedence among standard config files - it overrides both global and remote configs.
...
Specify a custom config file path using the `FLASHCODE_CONFIG` environment variable.

```bash
export FLASHCODE_CONFIG=/path/to/my/custom-config.json
...
Specify a custom config directory using the `FLASHCODE_CONFIG_DIR`
environment variable. This directory will be searched for agents, commands,
modes, and plugins just like the standard `.flashcode` directory, and should
follow the same structure.

```bash
export FLASHCODE_CONFIG_DIR=/path/to/my/config-directory
opencode run "Hello world"
```

The custom directory is loaded after the global config and `.flashcode` directories, so it **can override** their settings.
...
Drop an `flashcode.json` or `flashcode.jsonc` file in the system managed config directory:
...
On macOS, OpenCode reads managed preferences from the `ai.flashcode.managed` preference domain. Deploy a `.mobileconfig` via MDM (Jamf, Kandji, FleetDM) and the settings are enforced automatically.

OpenCode checks these paths:

1. `/Library/Managed Preferences/<user>/ai.flashcode.managed.plist`
2. `/Library/Managed Preferences/ai.flashcode.managed.plist`

The plist keys map directly to `flashcode.json` fields. MDM metadata keys (`PayloadUUID`, `PayloadType`, etc.) are stripped automatically.

**Creating a `.mobileconfig`**

Use the `ai.flashcode.managed` PayloadType. The OpenCode config keys go directly in the payload dict:
...
<dict>
  ...
  <array>
    <dict>
      <key>PayloadType</key>
      <string>ai.flashcode.managed</string>
      <key>PayloadIdentifier</key>
      <string>com.example.flashcode.config</string>
      <key>PayloadUUID</key>
  ...
  <key>PayloadIdentifier</key>
  <string>com.example.flashcode</string>
  <key>PayloadUUID</key>
  <string>GENERATE-YOUR-OWN-UUID</string>
  <key>PayloadVersion</key>
  <integer>1</integer>
...
Use `FLASHCODE_TUI_CONFIG` to point to a custom TUI config file.

Legacy `theme`, `keybinds`, and `tui` keys in `flashcode.json` are deprecated and automatically migrated when possible.
...
You can also define agents using markdown files in `~/.config/flashcode/agents/` or `.flashcode/agents/`. [Learn more here](/docs/agents).
...
You can also define commands using markdown files in `~/.config/flashcode/commands/` or `.flashcode/commands/`. [Learn more here](/docs/commands).
...
Place plugin files in `.flashcode/plugins/` or `~/.config/flashcode/plugins/`. You can also load plugins from npm through the `plugin` option.
