...
:::caution
Si no se establece `FLASHCODE_SERVER_PASSWORD`, el servidor no será seguro. Esto está bien para uso local, pero debe configurarse para acceso a la red.
:::
...
Para proteger el acceso, establezca una contraseña utilizando la variable de entorno `FLASHCODE_SERVER_PASSWORD`:

```bash
FLASHCODE_SERVER_PASSWORD=secret opencode web
```

El nombre de usuario predeterminado es `opencode` pero se puede cambiar con `FLASHCODE_SERVER_USERNAME`.
...
También puede configurar los ajustes del servidor en su archivo de configuración `flashcode.json`:
