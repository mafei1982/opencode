...
2. **Configuración global** (`~/.config/flashcode/flashcode.json`) - preferencias del usuario
3. **Configuración personalizada** (`FLASHCODE_CONFIG` env var): anulaciones personalizadas
4. **Configuración del proyecto** (`flashcode.json` en el proyecto): configuración específica del proyecto
5. Directorios **`.flashcode`** - agentes, comandos, complementos
6. **Configuración en línea** (`FLASHCODE_CONFIG_CONTENT` env var): anulaciones del tiempo de ejecución
...
:::note
Los directorios `.flashcode` y `~/.config/flashcode` usan **nombres en plural** para los subdirectorios: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/` y `themes/`. También se admiten nombres singulares (por ejemplo, `agent/`) para compatibilidad con versiones anteriores.
:::
...
Coloque su configuración global OpenCode en `~/.config/flashcode/flashcode.json`. Utilice la configuración global para las preferencias de todo el usuario, como temas, proveedores o combinaciones de teclas.
...
Agregue `flashcode.json` en la raíz de su proyecto. La configuración del proyecto tiene la mayor prioridad entre los archivos de configuración estándar: anula las configuraciones globales y remotas.
...
Especifique una ruta de archivo de configuración personalizada utilizando la variable de entorno `FLASHCODE_CONFIG`.
...
Especifique un directorio de configuración personalizado usando `FLASHCODE_CONFIG_DIR`
variable de entorno. En este directorio se buscarán agentes, comandos,
modos y complementos como el directorio estándar `.flashcode`, y debería
...
El directorio personalizado se carga después de los directorios global config y `.flashcode`, por lo que **puede anular** su configuración.
...
También puede definir agentes utilizando archivos de Markdown en `~/.config/flashcode/agents/` o `.flashcode/agents/`. [Más información aquí](/docs/agents).
...
También puede definir comandos utilizando archivos de Markdown en `~/.config/flashcode/commands/` o `.flashcode/commands/`. [Más información aquí](/docs/commands).
...
Coloque los archivos de complemento en `.flashcode/plugins/` o `~/.config/flashcode/plugins/`. También puedes cargar complementos desde npm a través de la opción `plugin`.
