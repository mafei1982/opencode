...
- **macOS/Linux**: `~/.config/flashcode/flashcode.jsonc` (o `~/.config/flashcode/flashcode.json`)
- **macOS/Linux** (instalaciones anteriores): `~/.local/share/flashcode/flashcode.jsonc`
- **Windows**: Presione `WIN+R` y pegue `%USERPROFILE%\.config\opencode\flashcode.jsonc`
...
- **macOS**: Buscador -> `Cmd+Shift+G` -> pegar `~/.cache/flashcode`
- **Linux**: eliminar `~/.cache/flashcode` (o ejecutar `rm -rf ~/.cache/flashcode`)
...
Si su `flashcode.json(c)` contiene una sección `server`, elimínela temporalmente y reinicie la aplicación de escritorio.
