...
- `.flashcode/plugins/` - Complementos a nivel de proyecto
- `~/.config/flashcode/plugins/` - Complementos globales
...
Los **complementos npm** se instalan automáticamente usando Bun al inicio. Los paquetes y sus dependencias se almacenan en caché en `~/.cache/flashcode/node_modules/`.
...
1. Configuración global (`~/.config/flashcode/flashcode.json`)
2. Configuración del proyecto (`flashcode.json`)
3. Directorio global de complementos (`~/.config/flashcode/plugins/`)
4. Directorio de complementos del proyecto (`.flashcode/plugins/`)
...
```json title=".flashcode/package.json"
{
  "dependencies": {
    "shescape": "^2.1.0"
  }
...
```ts title=".flashcode/plugins/my-plugin.ts"
...
```js title=".flashcode/plugins/example.js"
...
```js title=".flashcode/plugins/notification.js"
...
```javascript title=".flashcode/plugins/env-protection.js"
...
```javascript title=".flashcode/plugins/inject-env.js"
...
```ts title=".flashcode/plugins/custom-tools.ts"
...
```ts title=".flashcode/plugins/compaction.ts"
import type { Plugin } from "@opencode-ai/plugin"
...
```ts title=".flashcode/plugins/custom-compaction.ts"
import type { Plugin } from "@opencode-ai/plugin"
