...
It loads any matching `skills/*/SKILL.md` in `.flashcode/` and any matching `.claude/skills/*/SKILL.md` or `.agents/skills/*/SKILL.md` along the way.

Global definitions are also loaded from `~/.config/flashcode/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md`, and `~/.agents/skills/*/SKILL.md`.
...
Create `.flashcode/skills/git-release/SKILL.md` like this:
...
Control which skills agents can access using pattern-based permissions in `flashcode.json`:
...
**For built-in agents** (in `flashcode.json`):
