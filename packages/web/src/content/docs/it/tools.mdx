...
:::note
Questo strumento e' disponibile solo quando `FLASHCODE_EXPERIMENTAL_LSP_TOOL=true` (oppure `FLASHCODE_EXPERIMENTAL=true`).
:::
...
:::note
Questo strumento e' disponibile solo quando usi il provider OpenCode o quando la variabile d'ambiente `FLASHCODE_ENABLE_EXA` e' impostata a un valore truthy (ad esempio `true` o `1`).
