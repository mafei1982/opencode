...
- **macOS/Linux** (installazioni vecchie): `~/.local/share/flashcode/flashcode.jsonc`
- **Windows**: premi `WIN+R` e incolla `%USERPROFILE%\.config\opencode\flashcode.jsonc`
...
- **Linux**: elimina `~/.cache/flashcode` (oppure esegui `rm -rf ~/.cache/flashcode`)
...
Se il tuo `flashcode.json(c)` contiene una sezione `server`, rimuovila temporaneamente e riavvia l'app desktop.
