...
| <nobr><code>{"--password"}</code></nobr> | `-p`  | Password per l'autenticazione di base (predefinita: `FLASHCODE_SERVER_PASSWORD`)                 |
| <nobr><code>{"--username"}</code></nobr> | `-u`  | Nome utente per l'autenticazione di base (predefinito: `FLASHCODE_SERVER_USERNAME` o `opencode`) |
...
OpenCode si basa sull'elenco provider di [Models.dev](https://models.dev), quindi puoi usare `opencode auth login` per configurare le API key per qualunque provider tu voglia usare. Le credenziali vengono salvate in `~/.local/share/flashcode/auth.json`.
...
Avvia un server HTTP che espone accesso API alle funzionalità di opencode senza la TUI. Imposta `FLASHCODE_SERVER_PASSWORD` per abilitare HTTP basic auth (username di default `opencode`).
...
Avvia un server HTTP e apre un browser per accedere a OpenCode tramite interfaccia web. Imposta `FLASHCODE_SERVER_PASSWORD` per abilitare HTTP basic auth (username di default `opencode`).
...
| ------------------------------------- | ------- | ----------------------------------------------------------- |
| `FLASHCODE_AUTO_SHARE`                 | boolean | Condivide automaticamente le sessioni                       |
| `FLASHCODE_GIT_BASH_PATH`              | string  | Percorso all'eseguibile Git Bash su Windows                 |
| `FLASHCODE_CONFIG`                     | string  | Percorso al file di configurazione                          |
| `FLASHCODE_TUI_CONFIG`                 | string  | Percorso al file di configurazione TUI                      |
| `FLASHCODE_CONFIG_DIR`                 | string  | Percorso alla directory di configurazione                   |
| `FLASHCODE_CONFIG_CONTENT`             | string  | Contenuto JSON di config inline                             |
| `FLASHCODE_DISABLE_AUTOUPDATE`         | boolean | Disabilita i controlli automatici di aggiornamento          |
| `FLASHCODE_DISABLE_PRUNE`              | boolean | Disabilita la potatura dei dati vecchi                      |
| `FLASHCODE_DISABLE_TERMINAL_TITLE`     | boolean | Disabilita aggiornamenti automatici del titolo terminale    |
| `FLASHCODE_PERMISSION`                 | string  | Config permessi JSON inline                                 |
| `FLASHCODE_DISABLE_DEFAULT_PLUGINS`    | boolean | Disabilita i plugin di default                              |
| `FLASHCODE_DISABLE_LSP_DOWNLOAD`       | boolean | Disabilita download automatico dei server LSP               |
| `FLASHCODE_ENABLE_EXPERIMENTAL_MODELS` | boolean | Abilita modelli sperimentali                                |
| `FLASHCODE_DISABLE_AUTOCOMPACT`        | boolean | Disabilita compaction automatica del contesto               |
| `FLASHCODE_DISABLE_CLAUDE_CODE`        | boolean | Disabilita lettura da `.claude` (prompt + skill)            |
| `FLASHCODE_DISABLE_CLAUDE_CODE_PROMPT` | boolean | Disabilita lettura di `~/.claude/CLAUDE.md`                 |
| `FLASHCODE_DISABLE_CLAUDE_CODE_SKILLS` | boolean | Disabilita caricamento di `.claude/skills`                  |
| `FLASHCODE_DISABLE_MODELS_FETCH`       | boolean | Disabilita fetch dei modelli da fonti remote                |
| `FLASHCODE_FAKE_VCS`                   | string  | Provider VCS finto per scopi di test                        |
| `FLASHCODE_CLIENT`                     | string  | Identificatore client (default `cli`)                       |
| `FLASHCODE_ENABLE_EXA`                 | boolean | Abilita gli strumenti di web search Exa                     |
| `FLASHCODE_SERVER_PASSWORD`            | string  | Abilita basic auth per `serve`/`web`                        |
| `FLASHCODE_SERVER_USERNAME`            | string  | Sovrascrive lo username basic auth (default `opencode`)     |
| `FLASHCODE_MODELS_URL`                 | string  | URL personalizzato per recuperare la configurazione modelli |
...
| Variabile                                       | Tipo    | Descrizione                                |
| ----------------------------------------------- | ------- | ------------------------------------------ |
| `FLASHCODE_EXPERIMENTAL`                         | boolean | Abilita tutte le funzionalità sperimentali |
| `FLASHCODE_EXPERIMENTAL_ICON_DISCOVERY`          | boolean | Abilita icon discovery                     |
| `FLASHCODE_EXPERIMENTAL_DISABLE_COPY_ON_SELECT`  | boolean | Disabilita copy on select nella TUI        |
| `FLASHCODE_EXPERIMENTAL_BASH_DEFAULT_TIMEOUT_MS` | number  | Timeout di default per comandi bash in ms  |
| `FLASHCODE_EXPERIMENTAL_OUTPUT_TOKEN_MAX`        | number  | Massimo token di output per risposte LLM   |
| `FLASHCODE_EXPERIMENTAL_FILEWATCHER`             | boolean | Abilita file watcher per l'intera dir      |
| `FLASHCODE_EXPERIMENTAL_OXFMT`                   | boolean | Abilita formatter oxfmt                    |
| `FLASHCODE_EXPERIMENTAL_LSP_TOOL`                | boolean | Abilita strumento LSP sperimentale         |
| `FLASHCODE_EXPERIMENTAL_DISABLE_FILEWATCHER`     | boolean | Disabilita file watcher                    |
| `FLASHCODE_EXPERIMENTAL_EXA`                     | boolean | Abilita funzionalità Exa sperimentali      |
| `FLASHCODE_EXPERIMENTAL_LSP_TY`                  | boolean | Abilita TY LSP per i file python           |
| `FLASHCODE_EXPERIMENTAL_MARKDOWN`                | boolean | Abilita markdown sperimentale              |
| `FLASHCODE_EXPERIMENTAL_PLAN_MODE`               | boolean | Abilita plan mode                          |
