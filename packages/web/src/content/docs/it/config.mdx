...
2. **Config globale** (`~/.config/flashcode/flashcode.json`) - preferenze utente
3. **Config personalizzata** (variabile d'ambiente `FLASHCODE_CONFIG`) - sovrascritture personalizzate
4. **Config di progetto** (`flashcode.json` nel progetto) - impostazioni specifiche del progetto
5. **Directory `.flashcode`** - agenti, comandi, plugin
6. **Config inline** (variabile d'ambiente `FLASHCODE_CONFIG_CONTENT`) - sovrascritture a runtime
...
:::note
Le directory `.flashcode` e `~/.config/flashcode` usano **nomi al plurale** per le sottodirectory: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/` e `themes/`. I nomi al singolare (ad es. `agent/`) sono supportati per retrocompatibilita.
:::
...
Metti la configurazione globale di OpenCode in `~/.config/flashcode/flashcode.json`. Usa la configurazione globale per preferenze server/runtime valide per l'utente come provider, modelli e permessi.

Per le impostazioni specifiche della TUI, usa `~/.config/flashcode/tui.json`.
...
Aggiungi `flashcode.json` nella root del progetto. La configurazione di progetto ha la precedenza piu alta tra i file standard: sovrascrive sia la configurazione globale sia quella remota.
...
Specifica un percorso personalizzato per il file di configurazione usando la variabile d'ambiente `FLASHCODE_CONFIG`.
...
Specifica una directory di configurazione personalizzata usando la variabile d'ambiente `FLASHCODE_CONFIG_DIR`. Questa directory verra usata per cercare agenti, comandi, modalita e plugin proprio come la directory standard `.flashcode` e dovrebbe seguire la stessa struttura.
...
La directory personalizzata viene caricata dopo la configurazione globale e le directory `.flashcode`, quindi **puo sovrascrivere** le loro impostazioni.
...
Usa `FLASHCODE_TUI_CONFIG` per puntare a un file di configurazione TUI personalizzato.

Le chiavi legacy `theme`, `keybinds` e `tui` in `flashcode.json` sono deprecate e vengono migrate automaticamente quando possibile.
...
Puoi anche definire agenti usando file markdown in `~/.config/flashcode/agents/` o `.flashcode/agents/`. [Scopri di piu](/docs/agents).
...
Puoi anche definire comandi usando file markdown in `~/.config/flashcode/commands/` o `.flashcode/commands/`. [Scopri di piu](/docs/commands).
...
Metti i file dei plugin in `.flashcode/plugins/` o `~/.config/flashcode/plugins/`. Puoi anche caricare plugin da npm tramite l'opzione `plugin`.
