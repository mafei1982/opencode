...
:::caution
Wenn `FLASHCODE_SERVER_PASSWORD` nicht gesetzt ist, ist der Server ungesichert. Dies ist für den lokalen Gebrauch in Ordnung, sollte aber für den Netzwerkzugriff eingestellt werden.
:::
...
Um den Zugriff zu schützen, legen Sie mithilfe der Umgebungsvariablen `FLASHCODE_SERVER_PASSWORD` ein Passwort fest:

```bash
FLASHCODE_SERVER_PASSWORD=secret opencode web
```

Der Benutzername ist standardmäßig `opencode`, kann aber mit `FLASHCODE_SERVER_USERNAME` geändert werden.
...
Sie können Servereinstellungen auch in Ihrer `flashcode.json`-Konfigurationsdatei konfigurieren:
