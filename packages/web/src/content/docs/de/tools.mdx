...
:::note
Dieses Tool ist nur verfuegbar, wenn `FLASHCODE_EXPERIMENTAL_LSP_TOOL=true` (oder `FLASHCODE_EXPERIMENTAL=true`) gesetzt ist.
:::
...
:::note
Dieses Tool ist nur verfuegbar, wenn der OpenCode-Provider genutzt wird oder die Umgebungsvariable `FLASHCODE_ENABLE_EXA` auf einen 'truthy' Wert (z. B. `true` oder `1`) gesetzt ist.
