...
2. **Benutzer-Config-Verzeichnis** - Definiert in `~/.config/flashcode/themes/*.json` oder `$XDG_CONFIG_HOME/opencode/themes/*.json`
3. **Projekt-Root-Verzeichnis** - Definiert in `<project-root>/.flashcode/themes/*.json`
4. **Aktuelles Arbeitsverzeichnis** - Definiert in `./.flashcode/themes/*.json`
...
```bash no-frame
mkdir -p ~/.config/flashcode/themes
vim ~/.config/flashcode/themes/my-theme.json
```

Und fuer projektspezifische Themes:

```bash no-frame
mkdir -p .flashcode/themes
vim .flashcode/themes/my-theme.json
```
