...
2. **Globale Konfiguration** (`~/.config/flashcode/flashcode.json`) – Benutzereinstellungen
3. **Benutzerdefinierte Konfiguration** (`FLASHCODE_CONFIG` env var) – benutzerdefinierte Überschreibungen
4. **Projektkonfiguration** (`flashcode.json` im Projekt) – projektspezifische Einstellungen
5. **`.flashcode` Verzeichnisse** – Agenten, Befehle, Plugins
6. **Inline-Konfiguration** (`FLASHCODE_CONFIG_CONTENT` env var) – Laufzeitüberschreibungen
...
:::note
Die Verzeichnisse `.flashcode` und `~/.config/flashcode` verwenden **Pluralnamen** für Unterverzeichnisse: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/` und `themes/`. Aus Gründen der Abwärtskompatibilität werden auch einzelne Namen (z. B. `agent/`) unterstützt.
:::
...
Platzieren Sie Ihre globale OpenCode-Konfiguration in `~/.config/flashcode/flashcode.json`. Verwenden Sie die globale Konfiguration für benutzerweite Einstellungen wie Themen, Anbieter oder Tastenkombinationen.
...
Fügen Sie `flashcode.json` in Ihrem Projektstamm hinzu. Die Projektkonfiguration hat unter den Standardkonfigurationsdateien die höchste Priorität – sie überschreibt sowohl globale als auch Remote-Konfigurationen.
...
Geben Sie mithilfe der Umgebungsvariablen `FLASHCODE_CONFIG` einen benutzerdefinierten Konfigurationsdateipfad an.
...
Geben Sie mit `FLASHCODE_CONFIG_DIR` ein benutzerdefiniertes Konfigurationsverzeichnis an.
Umgebungsvariable. Dieses Verzeichnis wird nach Agenten, Befehlen,
Modi und Plugins genau wie das Standardverzeichnis `.flashcode` und sollten
...
Das benutzerdefinierte Verzeichnis wird nach den Verzeichnissen global config und `.flashcode` geladen, sodass es deren Einstellungen **überschreiben** kann.
...
Sie können Agenten auch mithilfe von Markdown-Dateien in `~/.config/flashcode/agents/` oder `.flashcode/agents/` definieren. [Mehr erfahren](/docs/agents).
...
Sie können Befehle auch mithilfe von Markdown-Dateien in `~/.config/flashcode/commands/` oder `.flashcode/commands/` definieren. [Mehr erfahren](/docs/commands).
...
Platzieren Sie Ihre Plugin-Dateien in `.flashcode/plugins/` oder `~/.config/flashcode/plugins/`. Sie können Plugins auch über die Option `plugin` von npm laden.
