...
Du kannst auch globale Regeln in `~/.config/flashcode/AGENTS.md` speichern.
...
- **Globale Regeln**: `~/.claude/CLAUDE.md` (wenn keine `~/.config/flashcode/AGENTS.md` existiert)
...
2. **Globale Datei** unter `~/.config/flashcode/AGENTS.md`
...
Du kannst zusaetzliche Anweisungsdateien in deiner `flashcode.json` oder in `~/.config/flashcode/flashcode.json` angeben.
...
Empfohlen ist das `instructions`-Feld in der `flashcode.json`:
...
:::tip
Fuer Monorepos oder gemeinsame Standards ist `flashcode.json` mit Glob-Mustern (z. B. `packages/*/AGENTS.md`) oft wartbarer als manuelle Anweisungen.
:::
