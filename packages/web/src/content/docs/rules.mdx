...
You can also have global rules in a `~/.config/flashcode/AGENTS.md` file. This gets applied across all opencode sessions.
...
- **Global rules**: `~/.claude/CLAUDE.md` (used if no `~/.config/flashcode/AGENTS.md` exists)
...
2. **Global file** at `~/.config/flashcode/AGENTS.md`
3. **Claude Code file** at `~/.claude/CLAUDE.md` (unless disabled)

The first matching file wins in each category. For example, if you have both `AGENTS.md` and `CLAUDE.md`, only `AGENTS.md` is used. Similarly, `~/.config/flashcode/AGENTS.md` takes precedence over `~/.claude/CLAUDE.md`.

---

## Custom Instructions

You can specify custom instruction files in your `flashcode.json` or the global `~/.config/flashcode/flashcode.json`. This allows you and your team to reuse existing rules rather than having to duplicate them to AGENTS.md.
...
### Using flashcode.json

The recommended approach is to use the `instructions` field in `flashcode.json`:
...
:::tip
For monorepos or projects with shared standards, using `flashcode.json` with glob patterns (like `packages/*/AGENTS.md`) is more maintainable than manual instructions.
:::
