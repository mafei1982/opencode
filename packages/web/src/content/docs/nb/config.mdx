...
2. **Global konfigurasjon** (`~/.config/flashcode/flashcode.json`) - brukerinnstillinger
3. **Egendefinert konfigurasjon** (`FLASHCODE_CONFIG` env var) - egendefinerte overstyringer
4. **Prosjektkonfigurasjon** (`flashcode.json` i prosjekt) - prosjektspesifikke innstillinger
5. **`.flashcode` kataloger** - agenter, kommandoer, plugins
6. **Inline config** (`FLASHCODE_CONFIG_CONTENT` env var) - kjøretidsoverstyringer
...
:::note
`.flashcode` og `~/.config/flashcode` katalogene bruker **flertallsnavn** på underkataloger: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/` og `themes/`. Enkeltnavn (f.eks. `agent/`) støttes også for bakoverkompatibilitet.
:::
...
Plasser din globale OpenCode-konfigurasjon i `~/.config/flashcode/flashcode.json`. Bruk global konfigurasjon for brukerspesifikke preferanser som temaer, leverandører eller nøkkelbindinger.
...
Legg til `flashcode.json` i prosjektroten din. Prosjektkonfigurasjon har den høyeste prioritet blant standard config-filer - den overstyrer både globale og eksterne konfigurasjoner.
...
Spesifiser en tilpasset konfigurasjonsfilbane ved å bruke miljøvariabelen `FLASHCODE_CONFIG`.
...
Spesifiser en tilpasset konfigurasjonskatalog ved å bruke `FLASHCODE_CONFIG_DIR`
miljøvariabel. Denne katalogen vil bli søkt etter agenter, kommandoer,
moduser og plugins akkurat som standard `.flashcode` katalog, og bør
...
Den egendefinerte katalogen lastes inn etter den globale konfigurasjonen og `.flashcode`-katalogene, slik at den **kan overstyre** innstillingene deres.
...
Du kan også definere agenter ved å bruke markdown-filer i `~/.config/flashcode/agents/` eller `.flashcode/agents/`. [Les mer her](/docs/agents).
...
Du kan også definere kommandoer ved å bruke markdown-filer i `~/.config/flashcode/commands/` eller `.flashcode/commands/`. [Les mer her](/docs/commands).
...
Plasser plugin-filer i `.flashcode/plugins/` eller `~/.config/flashcode/plugins/`. Du kan også laste inn plugins fra npm gjennom alternativet `plugin`.
