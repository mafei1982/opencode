...
For å beskytte tilgang, angi et passord ved hjelp av miljøvariabelen `FLASHCODE_SERVER_PASSWORD`:

```bash
FLASHCODE_SERVER_PASSWORD=secret opencode web
```

Brukernavnet er satt til `opencode` som standard, men kan endres med `FLASHCODE_SERVER_USERNAME`.
...
Du kan også konfigurere serverinnstillinger i `flashcode.json` konfigurasjonsfilen:
