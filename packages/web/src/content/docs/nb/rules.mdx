...
Du kan også ha globale regler i en `~/.config/flashcode/AGENTS.md`-fil. Dette blir brukt på alle OpenCode-økter.
...
- **Globale regler**: `~/.claude/CLAUDE.md` (brukes hvis ingen `~/.config/flashcode/AGENTS.md` eksisterer)
...
Den første matchende filen vinner i hver kategori. For eksempel, hvis du har både `AGENTS.md` og `CLAUDE.md`, brukes bare `AGENTS.md`. På samme måte har `~/.config/flashcode/AGENTS.md` forrang over `~/.claude/CLAUDE.md`.

---

## Egendefinerte instruksjoner

Du kan spesifisere egendefinerte instruksjonsfiler i din `flashcode.json` eller den globale `~/.config/flashcode/flashcode.json`. Dette lar deg og teamet ditt gjenbruke eksisterende regler i stedet for å måtte duplisere dem til AGENTS.md.
...
### Bruker flashcode.json

Den anbefalte tilnærmingen er å bruke feltet `instructions` i `flashcode.json`:
...
:::tip
For monorepos eller prosjekter med delte standarder er bruk av `flashcode.json` med glob-mønstre (som `packages/*/AGENTS.md`) mer vedlikeholdbart enn manuelle instruksjoner.
:::
