...
i `~/.local/share/flashcode/auth.json`.
...
2. **Konfigurer autentisering** ved å bruke en av følgende metoder:
   ...
   For prosjektspesifikk eller vedvarende konfigurasjon, bruk `flashcode.json`:
...
`flashcode.json` fil. Det anbefales også å deaktivere øktdeling.
...
Tilpass gjennom `flashcode.json`:
...
4. Opprett eller oppdater `flashcode.json`-filen i prosjektkatalogen:
