...
- **macOS/Linux** (eldre installasjoner): `~/.local/share/flashcode/flashcode.jsonc`
- **Windows**: Trykk `WIN+R` og lim inn `%USERPROFILE%\.config\opencode\flashcode.jsonc`
...
- **macOS**: Finder -> `Cmd+Shift+G` -> lim inn `~/.cache/flashcode`
- **Linux**: slett `~/.cache/flashcode` (eller kjør `rm -rf ~/.cache/flashcode`)
...
Hvis `flashcode.json(c)` inneholder en `server`-del, fjern den midlertidig og start skrivebordsappen på nytt.
