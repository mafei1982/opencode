...
:::note
Dette verktøyet er bare tilgjengelig når `FLASHCODE_EXPERIMENTAL_LSP_TOOL=true` (eller `FLASHCODE_EXPERIMENTAL=true`).
:::
...
:::note
Dette verktøyet er bare tilgjengelig når du bruker OpenCode-leverandøren eller når miljøvariabelen `FLASHCODE_ENABLE_EXA` er satt til en sann verdi (f.eks. `true` eller `1`).
