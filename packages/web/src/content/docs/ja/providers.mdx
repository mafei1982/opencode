...
`~/.local/share/flashcode/auth.json`で。
...
2. **次のいずれかの方法を使用して認証を構成します**:
   ...
   ***
   ...
   プロジェクト固有の設定または永続的な設定の場合は、`flashcode.json` を使用します。
...
`flashcode.json` ファイル。セッション共有を無効にすることもお勧めします。
...
4. プロジェクトディレクトリで `flashcode.json` ファイルを作成または更新します。
