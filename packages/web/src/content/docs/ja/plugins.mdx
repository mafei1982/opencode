...
- `.flashcode/plugins/` - プロジェクトレベルのプラグイン
- `~/.config/flashcode/plugins/` - グローバルプラグイン
...
**npm プラグイン** は、起動時に Bun を使用して自動的にインストールされます。パッケージとその依存関係は `~/.cache/flashcode/node_modules/` にキャッシュされます。
...
1. グローバル設定 (`~/.config/flashcode/flashcode.json`)
2. プロジェクト設定 (`flashcode.json`)
3. グローバルプラグインディレクトリ (`~/.config/flashcode/plugins/`)
4. プロジェクトプラグインディレクトリ (`.flashcode/plugins/`)
