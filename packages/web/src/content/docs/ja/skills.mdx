...
- プロジェクト設定: `.flashcode/skills/<name>/SKILL.md`
- グローバル設定: `~/.config/flashcode/skills/<name>/SKILL.md`
...
途中で、一致する `skills/*/SKILL.md` を `.flashcode/` に読み込み、一致する `.claude/skills/*/SKILL.md` または `.agents/skills/*/SKILL.md` を読み込みます。

グローバル定義は、`~/.config/flashcode/skills/*/SKILL.md`、`~/.claude/skills/*/SKILL.md`、および `~/.agents/skills/*/SKILL.md` からもロードされます。
...
次のように `.flashcode/skills/git-release/SKILL.md` を作成します。
...
`flashcode.json` のパターンベースの権限を使用して、エージェントがアクセスできるスキルを制御します。
...
**組み込みエージェントの場合** (`flashcode.json` 内):
