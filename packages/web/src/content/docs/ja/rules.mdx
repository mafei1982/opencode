...
`~/.config/flashcode/AGENTS.md` ファイルにグローバルルールを含めることもできます。これは、すべての OpenCode セッションに適用されます。
...
- **グローバルルール**: `~/.claude/CLAUDE.md` (`~/.config/flashcode/AGENTS.md` が存在しない場合に使用)
...
2. **グローバルファイル** (`~/.config/flashcode/AGENTS.md`)
3. **Claude Code ファイル** (`~/.claude/CLAUDE.md` にあります) (無効になっていない限り)

最初に一致したファイルが各カテゴリで優先されます。たとえば、`AGENTS.md` と `CLAUDE.md` の両方がある場合、`AGENTS.md` のみが使用されます。同様に、`~/.config/flashcode/AGENTS.md` は `~/.claude/CLAUDE.md` よりも優先されます。

---

## カスタム指示

`flashcode.json` またはグローバル `~/.config/flashcode/flashcode.json` でカスタム命令ファイルを指定できます。これにより、あなたとあなたのチームは、既存のルールを AGENTS.md に複製するのではなく、再利用することができます。
...
推奨されるアプローチは、`instructions` の `flashcode.json` フィールドを使用することです。
...
:::tip
モノリポジトリまたは共有標準を使用するプロジェクトの場合、グロブパターン (`flashcode.json` など) で `packages/*/AGENTS.md` を使用する方が、手動で指示するよりも保守しやすくなります。
:::
