...
- **macOS/Linux**: `~/.config/flashcode/flashcode.jsonc` (または `~/.config/flashcode/flashcode.json`)
- **macOS/Linux** (古いインストール): `~/.local/share/flashcode/flashcode.jsonc`
- **Windows**: `WIN+R` を押して `%USERPROFILE%\.config\opencode\flashcode.jsonc` を貼り付けます
...
- **macOS**: Finder -> `Cmd+Shift+G` -> `~/.cache/flashcode` を貼り付け
- **Linux**: `~/.cache/flashcode` を削除します (または `rm -rf ~/.cache/flashcode` を実行します)。
...
`flashcode.json(c)` に `server` セクションが含まれている場合は、それを一時的に削除し、デスクトップアプリを再起動します。
