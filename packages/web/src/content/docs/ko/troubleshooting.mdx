...
-**macOS/리눅스**: `~/.local/share/flashcode/log/`
...
-**macOS/리눅스**: `~/.local/share/flashcode/`
...
-**macOS/리눅스**: `~/.config/flashcode/flashcode.jsonc` (또는 `~/.config/flashcode/flashcode.json`) -**macOS/Linux** (외부 설치): `~/.local/share/flashcode/flashcode.jsonc`

- **Windows**: 압박 `WIN+R`와 풀 `%USERPROFILE%\.config\opencode\flashcode.jsonc`
...
- **글로벌 플러그인** -**macOS/리눅스**: `~/.config/flashcode/plugins/`
...
-**macOS**: 찾기 -> `Cmd+Shift+G` -> 붙여넣기 `~/.cache/flashcode`

- **Linux**: `~/.cache/flashcode` 삭제 (또는 `rm -rf ~/.cache/flashcode` 실행)
...
`flashcode.json(c)`가 `server` 섹션을 포함하면 일시적으로 제거하고 데스크톱 앱을 다시 시작합니다.
