...
:::note
This tool is only available when `FLASHCODE_EXPERIMENTAL_LSP_TOOL=true` (or `FLASHCODE_EXPERIMENTAL=true`).
:::
...
:::note
This tool is only available when using the OpenCode provider or when the `FLASHCODE_ENABLE_EXA` environment variable is set to any truthy value (e.g., `true` or `1`).
