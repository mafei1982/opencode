...
| Flag                                        | Short | Description                                                                                                                                                                                                                |
| ------------------------------------------- | ----- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| <nobr><code>{"--path"}</code></nobr>        |       | Directory to write the agent file to (defaults to global or `.flashcode/agent` based on the prompt)                                                                                                                         |
...
| <nobr><code>{"--session"}</code></nobr>  | `-s`  | Session ID to continue                                                     |
| <nobr><code>{"--fork"}</code></nobr>     |       | Fork the session when continuing (use with `--continue` or `--session`)    |
| <nobr><code>{"--password"}</code></nobr> | `-p`  | Basic auth password (defaults to `FLASHCODE_SERVER_PASSWORD`)               |
| <nobr><code>{"--username"}</code></nobr> | `-u`  | Basic auth username (defaults to `FLASHCODE_SERVER_USERNAME` or `opencode`) |
...
OpenCode is powered by the provider list at [Models.dev](https://models.dev), so you can use `opencode auth login` to configure API keys for any provider you'd like to use. This is stored in `~/.local/share/flashcode/auth.json`.
...
This starts an HTTP server that provides API access to opencode functionality without the TUI interface. Set `FLASHCODE_SERVER_PASSWORD` to enable HTTP basic auth (username defaults to `opencode`).
...
This starts an HTTP server and opens a web browser to access OpenCode through a web interface. Set `FLASHCODE_SERVER_PASSWORD` to enable HTTP basic auth (username defaults to `opencode`).
...
| ------------------------------------- | ------- | ------------------------------------------------- |
| `FLASHCODE_AUTO_SHARE`                 | boolean | Automatically share sessions                      |
| `FLASHCODE_GIT_BASH_PATH`              | string  | Path to Git Bash executable on Windows            |
| `FLASHCODE_CONFIG`                     | string  | Path to config file                               |
| `FLASHCODE_TUI_CONFIG`                 | string  | Path to TUI config file                           |
| `FLASHCODE_CONFIG_DIR`                 | string  | Path to config directory                          |
| `FLASHCODE_CONFIG_CONTENT`             | string  | Inline json config content                        |
| `FLASHCODE_DISABLE_AUTOUPDATE`         | boolean | Disable automatic update checks                   |
| `FLASHCODE_DISABLE_PRUNE`              | boolean | Disable pruning of old data                       |
| `FLASHCODE_DISABLE_TERMINAL_TITLE`     | boolean | Disable automatic terminal title updates          |
| `FLASHCODE_PERMISSION`                 | string  | Inlined json permissions config                   |
| `FLASHCODE_DISABLE_DEFAULT_PLUGINS`    | boolean | Disable default plugins                           |
| `FLASHCODE_DISABLE_LSP_DOWNLOAD`       | boolean | Disable automatic LSP server downloads            |
| `FLASHCODE_ENABLE_EXPERIMENTAL_MODELS` | boolean | Enable experimental models                        |
| `FLASHCODE_DISABLE_AUTOCOMPACT`        | boolean | Disable automatic context compaction              |
| `FLASHCODE_DISABLE_CLAUDE_CODE`        | boolean | Disable reading from `.claude` (prompt + skills)  |
| `FLASHCODE_DISABLE_CLAUDE_CODE_PROMPT` | boolean | Disable reading `~/.claude/CLAUDE.md`             |
| `FLASHCODE_DISABLE_CLAUDE_CODE_SKILLS` | boolean | Disable loading `.claude/skills`                  |
| `FLASHCODE_DISABLE_MODELS_FETCH`       | boolean | Disable fetching models from remote sources       |
| `FLASHCODE_DISABLE_MOUSE`              | boolean | Disable mouse capture in the TUI                  |
| `FLASHCODE_FAKE_VCS`                   | string  | Fake VCS provider for testing purposes            |
| `FLASHCODE_CLIENT`                     | string  | Client identifier (defaults to `cli`)             |
| `FLASHCODE_ENABLE_EXA`                 | boolean | Enable Exa web search tools                       |
| `FLASHCODE_SERVER_PASSWORD`            | string  | Enable basic auth for `serve`/`web`               |
| `FLASHCODE_SERVER_USERNAME`            | string  | Override basic auth username (default `opencode`) |
| `FLASHCODE_MODELS_URL`                 | string  | Custom URL for fetching models configuration      |
...
| Variable                                        | Type    | Description                             |
| ----------------------------------------------- | ------- | --------------------------------------- |
| `FLASHCODE_EXPERIMENTAL`                         | boolean | Enable all experimental features        |
| `FLASHCODE_EXPERIMENTAL_ICON_DISCOVERY`          | boolean | Enable icon discovery                   |
| `FLASHCODE_EXPERIMENTAL_DISABLE_COPY_ON_SELECT`  | boolean | Disable copy on select in TUI           |
| `FLASHCODE_EXPERIMENTAL_BASH_DEFAULT_TIMEOUT_MS` | number  | Default timeout for bash commands in ms |
| `FLASHCODE_EXPERIMENTAL_OUTPUT_TOKEN_MAX`        | number  | Max output tokens for LLM responses     |
| `FLASHCODE_EXPERIMENTAL_FILEWATCHER`             | boolean | Enable file watcher for entire dir      |
| `FLASHCODE_EXPERIMENTAL_OXFMT`                   | boolean | Enable oxfmt formatter                  |
| `FLASHCODE_EXPERIMENTAL_LSP_TOOL`                | boolean | Enable experimental LSP tool            |
| `FLASHCODE_EXPERIMENTAL_DISABLE_FILEWATCHER`     | boolean | Disable file watcher                    |
| `FLASHCODE_EXPERIMENTAL_EXA`                     | boolean | Enable experimental Exa features        |
| `FLASHCODE_EXPERIMENTAL_LSP_TY`                  | boolean | Enable TY LSP for python files          |
| `FLASHCODE_EXPERIMENTAL_MARKDOWN`                | boolean | Enable experimental markdown features   |
| `FLASHCODE_EXPERIMENTAL_PLAN_MODE`               | boolean | Enable plan mode                        |
