...
2. **Konfiguracja globalna** (`~/.config/flashcode/flashcode.json`) – preferencje użytkownika
3. **Konfiguracja niestandardowa** (zmienna `FLASHCODE_CONFIG`) — jawne nadpisanie
4. **Konfiguracja projektu** (`flashcode.json` w projekcie) - ustawienia specyficzne dla projektu
5. **Katalogi `.flashcode`** - agenci, polecenia, umiejętności
6. **Konfiguracja wbudowana** (zmienna `FLASHCODE_CONFIG_CONTENT`) — nadpisanie środowiska uruchomieniowego
...
:::note
W katalogach `.flashcode` i `~/.config/flashcode` używane są **nazwy w liczbie mnogiej** dla podkatalogów: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/` i `themes/`. Nazwy w liczbie pojedynczej (np. `agent/`) są również obsługiwane dla kompatybilności wstecznej.
:::
...
Umieść swoją globalną konfigurację OpenCode w `~/.config/flashcode/flashcode.json`. Użyj jej do ustawień ogólnych dla użytkownika, takich jak dostawcy, modele i uprawnienia.

Dla ustawień specyficznych dla TUI, użyj `~/.config/flashcode/tui.json`.
...
Dodaj `flashcode.json` w katalogu głównym projektu. Konfiguracja projektu ma najwyższy priorytet wśród plików konfiguracyjnych — nadpisuje konfiguracje globalne i zdalne.
...
Możesz załadować niestandardowy plik konfiguracyjny, ustawiając zmienną środowiskową `FLASHCODE_CONFIG`.
...
Możesz ustawić niestandardowy katalog konfiguracyjny za pomocą zmiennej środowiskowej `FLASHCODE_CONFIG_DIR`. Katalog ten będzie przeszukiwany pod kątem agentów, poleceń, trybów i wtyczek, tak jak standardowy katalog `.flashcode` i powinien zachować tę samą strukturę.
...
Katalog niestandardowy jest ładowany po globalnej konfiguracji i katalogach `.flashcode`, więc **może być łączony** z ich zawartością.
...
Użyj `FLASHCODE_TUI_CONFIG`, aby wskazać niestandardowy plik konfiguracyjny TUI.

Przestarzałe klucze `theme`, `keybinds` i `tui` w `flashcode.json` są wycofywane i automatycznie migrowane, gdy to możliwe.
...
Możesz także definiować agentów przy użyciu plików markdown w `~/.config/flashcode/agents/` lub `.flashcode/agents/`. [Dowiedz się więcej o agentach](/docs/agents).
...
Możesz także definiować polecenia przy użyciu plików Markdown w `~/.config/flashcode/commands/` lub `.flashcode/commands/`. [Dowiedz się więcej o poleceniach](/docs/commands).
...
Umieść pliki wtyczek w `.flashcode/plugins/` lub `~/.config/flashcode/plugins/`. Możesz także załadować wtyczkę z npm za pomocą opcji `plugin`.
