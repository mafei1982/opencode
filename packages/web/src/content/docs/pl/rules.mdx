...
Możesz także mieć reguły globalne w pliku `~/.config/flashcode/AGENTS.md`. Jest to stosowane we wszystkich sesjach opencode.
...
- **Zasady globalne**: `~/.claude/CLAUDE.md` (używane, jeśli nie istnieje `~/.config/flashcode/AGENTS.md`)
...
2. **Plik globalny** w `~/.config/flashcode/AGENTS.md`
3. **Plik Claude Code** w `~/.claude/CLAUDE.md` (chyba że wyłączony)

Pierwszy pasujący plik wygrywa w każdej kategorii. Na przykład, jeśli masz zarówno `AGENTS.md`, jak i `CLAUDE.md`, używany jest tylko `AGENTS.md`. Podobnie `~/.config/flashcode/AGENTS.md` ma pierwszeństwo przed `~/.claude/CLAUDE.md`.

---

## Instrukcje niestandardowe

Możesz określić niestandardowe pliki instrukcji w swoim `flashcode.json` lub globalnym `~/.config/flashcode/flashcode.json`. Dzięki temu Ty i Twój zespół możecie ponownie wykorzystać istniejące reguły, zamiast duplikować je w pliku AGENTS.md.
...
### Używanie flashcode.json

Zalecanym podejściem jest użycie pola `instructions` w `flashcode.json`:
...
:::tip
W przypadku monorepo lub projektów ze wspólnymi standardami użycie `flashcode.json` ze wzorami globalnymi (takimi jak `packages/*/AGENTS.md`) jest łatwiejsze w utrzymaniu niż instrukcje ręczne.
:::
