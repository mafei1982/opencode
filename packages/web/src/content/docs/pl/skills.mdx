...
- Project config: `.flashcode/skills/<name>/SKILL.md`
- Global config: `~/.config/flashcode/skills/<name>/SKILL.md`
...
Ładuje po drodze dowolne pasujące `skills/*/SKILL.md` w `.flashcode/` i dowolne pasujące `.claude/skills/*/SKILL.md` lub `.agents/skills/*/SKILL.md`.

Ładowane są także definicje globalne z `~/.config/flashcode/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` i `~/.agents/skills/*/SKILL.md`.
...
Utwórz `.flashcode/skills/git-release/SKILL.md` w ten sposób:
...
Kontroluj, do których umiejętności agenci mogą uzyskać dostęp, używając uprawnień opartych na wzorcach w `flashcode.json`:
...
**Dla agentów wbudowanych** (w `flashcode.json`):
