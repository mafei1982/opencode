...
:::note
To narzędzie jest dostępne tylko wtedy, gdy `FLASHCODE_EXPERIMENTAL_LSP_TOOL=true` (lub `FLASHCODE_EXPERIMENTAL=true`).
:::
...
:::note
To narzędzie jest dostępne tylko w przypadku korzystania z dostawcy OpenCode lub gdy zmienna środowiskowa `FLASHCODE_ENABLE_EXA` jest ustawiona na dowolną wartość prawdziwą (np. `true` lub `1`).
