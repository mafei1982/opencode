...
- **macOS/Linux**: `~/.config/flashcode/flashcode.jsonc` (lub `~/.config/flashcode/flashcode.json`)
- **macOS/Linux** (starsze instalacje): `~/.local/share/flashcode/flashcode.jsonc`
- **Windows**: Naciśnij `WIN+R` i wklej `%USERPROFILE%\.config\opencode\flashcode.jsonc`
...
- **macOS**: Finder -> `Cmd+Shift+G` -> paste `~/.cache/flashcode`
- **Linux**: usuń `~/.cache/flashcode` (lub uruchom `rm -rf ~/.cache/flashcode`)
...
Jeśli Twój `flashcode.json(c)` zawiera sekcję `server`, tymczasowo usuń ją i uruchom ponownie aplikację komputerową.
