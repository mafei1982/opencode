...
2. **Katalog konfiguracji użytkownika** - Zdefiniowany w `~/.config/flashcode/themes/*.json` lub `$XDG_CONFIG_HOME/opencode/themes/*.json`
3. **Katalog główny projektu** - Zdefiniowany w `<project-root>/.flashcode/themes/*.json`
4. **Bieżący katalog roboczy** - Zdefiniowany w `./.flashcode/themes/*.json`
