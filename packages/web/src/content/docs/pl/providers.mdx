...
w `~/.local/share/flashcode/auth.json`.
...
2. **Skonfiguruj uwierzytelnianie** przy użyciu jednej z następujących metod:
   ...
   W przypadku konfiguracji specyficznej dla projektu lub trwałej użyj `flashcode.json`:
...
`flashcode.json` plik. Zalecane jest również wyłączenie udostępniania sesji.
...
4. Utwórz lub zaktualizuj plik `flashcode.json` w katalogu projektu:
