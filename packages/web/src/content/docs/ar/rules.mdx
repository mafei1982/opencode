...
يمكنك أيضًا وضع قواعد عامة في ملف `~/.config/flashcode/AGENTS.md`. تُطبَّق هذه القواعد على جميع جلسات opencode.
...
- **قواعد عامة**: `~/.claude/CLAUDE.md` (يُستخدم إذا لم يوجد `~/.config/flashcode/AGENTS.md`)
...
2. **ملف عام** في `~/.config/flashcode/AGENTS.md`
3. **ملف Claude Code** في `~/.claude/CLAUDE.md` (ما لم يتم تعطيله)

يتم اعتماد أول ملف مطابق ضمن كل فئة. على سبيل المثال، إذا كان لديك كل من `AGENTS.md` و`CLAUDE.md` فسيتم استخدام `AGENTS.md` فقط. وبالمثل، تكون الأولوية لملف `~/.config/flashcode/AGENTS.md` على `~/.claude/CLAUDE.md`.

---

## تعليمات مخصصة

يمكنك تحديد ملفات تعليمات مخصصة في `flashcode.json` أو في الملف العام `~/.config/flashcode/flashcode.json`. يتيح ذلك لك ولفريقك إعادة استخدام القواعد الموجودة بدلًا من تكرارها داخل AGENTS.md.
...
### استخدام flashcode.json

النهج الموصى به هو استخدام الحقل `instructions` في `flashcode.json`:
...
:::tip
بالنسبة للمستودعات متعددة الحزم (monorepos) أو المشاريع ذات المعايير المشتركة، فإن استخدام `flashcode.json` مع أنماط glob (مثل `packages/*/AGENTS.md`) أسهل للصيانة من التعليمات اليدوية.
:::
