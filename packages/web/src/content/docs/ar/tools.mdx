...
:::note
هذه الأداة متاحة فقط عند ضبط `FLASHCODE_EXPERIMENTAL_LSP_TOOL=true` (أو `FLASHCODE_EXPERIMENTAL=true`).
:::
...
:::note
هذه الأداة متاحة فقط عند استخدام مزود OpenCode أو عند ضبط متغير البيئة `FLASHCODE_ENABLE_EXA` على أي قيمة منطقية صحيحة (مثل `true` أو `1`).
