...
في `~/.local/share/flashcode/auth.json`.
...
2. **اضبط المصادقة** باستخدام إحدى الطرق التالية:
   ...
   للإعداد الخاص بالمشروع أو الإعداد الدائم، استخدم `flashcode.json`:
...
`flashcode.json` لديك. يوصى أيضا بتعطيل مشاركة الجلسات.
...
4. أنشئ أو حدّث ملف `flashcode.json` في دليل مشروعك:
