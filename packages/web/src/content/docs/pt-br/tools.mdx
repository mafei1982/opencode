...
:::note
Esta ferramenta está disponível apenas quando `FLASHCODE_EXPERIMENTAL_LSP_TOOL=true` (ou `FLASHCODE_EXPERIMENTAL=true`).
:::
...
:::note
Esta ferramenta está disponível apenas ao usar o provedor opencode ou quando a variável de ambiente `FLASHCODE_ENABLE_EXA` está definida como qualquer valor verdadeiro (por exemplo, `true` ou `1`).
