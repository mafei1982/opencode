...
- **macOS/Linux** (instalações mais antigas): `~/.local/share/flashcode/flashcode.jsonc`
- **Windows**: Pressione `WIN+R` e cole `%USERPROFILE%\.config\opencode\flashcode.jsonc`
...
- **macOS**: Finder -> `Cmd+Shift+G` -> cole `~/.cache/flashcode`
- **Linux**: exclua `~/.cache/flashcode` (ou execute `rm -rf ~/.cache/flashcode`)
...
Se seu `flashcode.json(c)` contiver uma seção `server`, remova-a temporariamente e reinicie o aplicativo de desktop.
