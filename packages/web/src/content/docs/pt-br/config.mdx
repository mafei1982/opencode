...
2. **Configuração global** (`~/.config/flashcode/flashcode.json`) - preferências do usuário
3. **Configuração personalizada** (`FLASHCODE_CONFIG` var de ambiente) - substituições personalizadas
4. **Configuração do projeto** (`flashcode.json` no projeto) - configurações específicas do projeto
5. **Diretórios `.flashcode`** - agentes, comandos, plugins
6. **Configuração inline** (`FLASHCODE_CONFIG_CONTENT` var de ambiente) - substituições em tempo de execução
...
:::note
Os diretórios `.flashcode` e `~/.config/flashcode` usam **nomes no plural** para subdiretórios: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/`, e `themes/`. Nomes no singular (por exemplo, `agent/`) também são suportados para compatibilidade retroativa.
:::
...
Coloque sua configuração global do opencode em `~/.config/flashcode/flashcode.json`. Use a configuração global para preferências de todo o usuário, como provedores, modelos e permissões.

Para configurações específicas do TUI, use `~/.config/flashcode/tui.json`.
...
Adicione `flashcode.json` na raiz do seu projeto. A configuração do projeto tem a maior precedência entre os arquivos de configuração padrão - ela substitui tanto as configurações globais quanto as remotas.
...
Especifique um caminho de arquivo de configuração personalizado usando a variável de ambiente `FLASHCODE_CONFIG`.
...
Especifique um diretório de configuração personalizado usando a variável de ambiente `FLASHCODE_CONFIG_DIR`. Este diretório será pesquisado por agentes, comandos, modos e plugins, assim como o diretório padrão `.flashcode`, e deve seguir a mesma estrutura.
...
O diretório personalizado é carregado após a configuração global e os diretórios `.flashcode`, então ele **pode substituir** suas configurações.
...
Use `FLASHCODE_TUI_CONFIG` para apontar para um arquivo de configuração TUI personalizado.

Chaves legadas `theme`, `keybinds` e `tui` em `flashcode.json` estão obsoletas e são migradas automaticamente quando possível.
...
Você também pode definir agentes usando arquivos markdown em `~/.config/flashcode/agents/` ou `.flashcode/agents/`. [Saiba mais aqui](/docs/agents).
...
Você também pode definir comandos usando arquivos markdown em `~/.config/flashcode/commands/` ou `.flashcode/commands/`. [Saiba mais aqui](/docs/commands).
...
Coloque arquivos de plugin em `.flashcode/plugins/` ou `~/.config/flashcode/plugins/`. Você também pode carregar plugins do npm através da opção `plugin`.
