...
- Configuração do projeto: `.flashcode/skills/<name>/SKILL.md`
- Configuração global: `~/.config/flashcode/skills/<name>/SKILL.md`
...
Ele carrega qualquer `skills/*/SKILL.md` correspondente em `.flashcode/` e qualquer `.claude/skills/*/SKILL.md` ou `.agents/skills/*/SKILL.md` ao longo do caminho.

As definições globais também são carregadas de `~/.config/flashcode/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` e `~/.agents/skills/*/SKILL.md`.
...
Crie `.flashcode/skills/git-release/SKILL.md` assim:
...
Controle quais habilidades os agentes podem acessar usando permissões baseadas em padrões em `flashcode.json`:
...
**Para agentes embutidos** (em `flashcode.json`):
