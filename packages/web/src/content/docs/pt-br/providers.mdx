...
Quando você adiciona as chaves da API de um provedor com o comando `/connect`, elas são armazenadas em `~/.local/share/flashcode/auth.json`.
...
2. **Configure a autenticação** usando um dos seguintes métodos:
   ...
   ***
   ...
   Para configuração específica do projeto ou persistente, use `flashcode.json`:
...
arquivo `flashcode.json`. Também é recomendado desativar o compartilhamento de sessões.
...
Personalize através do `flashcode.json`:
...
3. Insira sua chave da API para o provedor.
   ...
   ┌  Adicionar credencial
   │
   ▲  Isso armazena apenas uma credencial para myprovider - você precisará configurá-lo no flashcode.json, verifique a documentação para exemplos.
   │
...
4. Crie ou atualize seu arquivo `flashcode.json` no diretório do seu projeto:
