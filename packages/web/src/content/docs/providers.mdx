...
in `~/.local/share/flashcode/auth.json`.
...
2. **Configure authentication** using one of the following methods:
   ...
   ***
   ...
   For project-specific or persistent configuration, use `flashcode.json`:
...
```json title="flashcode.json" "atomic-chat" {5, 6, 8, 10-14}
{
  "$schema": "https://opencode.ai/config.json",
  "provider": {
    "atomic-chat": {
...
`flashcode.json` file. It is also recommended to disable session sharing.
...
Customize through `flashcode.json`:
...
```jsonc title="~/.config/flashcode/flashcode.jsonc"
{
  "$schema": "https://opencode.ai/config.json",
  "provider": {
    "helicone": {
...
When using the built-in `local_tcp` provider on Windows x64, opencode can manage the llama.cpp server binary for you. Set `LLM_TCP_SERVER_PATH` to use your own `llama-server.exe`; otherwise opencode checks packaged/build-time locations and downloads the pinned server on first use when it is missing.

```json title="flashcode.json" "llama.cpp" {5, 6, 8, 10-15}
{
  "$schema": "https://opencode.ai/config.json",
  "provider": {
    "llama.cpp": {
...
```json title="flashcode.json" "lmstudio" {5, 6, 8, 10-14}
{
  "$schema": "https://opencode.ai/config.json",
  "provider": {
    "lmstudio": {
...
```json title="flashcode.json" "ollama" {5, 6, 8, 10-14}
{
  "$schema": "https://opencode.ai/config.json",
  "provider": {
    "ollama": {
...
3. Enter your API key for the provider.
   ...
   ┌  Add credential
   │
   ▲  This only stores a credential for myprovider - you will need to configure it in flashcode.json, check the docs for examples.
   │
   ◇  Enter your API key
   │  sk-...
   └
...
4. Create or update your `flashcode.json` file in your project directory:

   ```json title="flashcode.json" ""myprovider"" {5-15}
   {
     "$schema": "https://opencode.ai/config.json",
     "provider": {
       "myprovider": {
...
```json title="flashcode.json" {9,11,17-20}
{
  "$schema": "https://opencode.ai/config.json",
  "provider": {
    "myprovider": {
