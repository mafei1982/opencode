...
- **macOS/Linux**（旧版安装）: `~/.local/share/flashcode/flashcode.jsonc`
- **Windows**: 按 `WIN+R` 并粘贴 `%USERPROFILE%\.config\opencode\flashcode.jsonc`
...
- **macOS**: Finder -> `Cmd+Shift+G` -> 粘贴 `~/.cache/flashcode`
- **Linux**: 删除 `~/.cache/flashcode`（或运行 `rm -rf ~/.cache/flashcode`）
...
如果你的 `flashcode.json(c)` 包含 `server` 部分，请临时移除该部分并重新启动桌面应用。
