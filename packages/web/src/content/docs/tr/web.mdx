...
:::caution
`FLASHCODE_SERVER_PASSWORD` ayarlanmadıysa sunucunun güvenliği kaldırılacaktır. Bu, yerel kullanım için iyidir ancak ağ erişimi için ayarlanmalıdır.
:::
...
Erişimi korumak için `FLASHCODE_SERVER_PASSWORD` ortam değişkenini kullanarak bir parola ayarlayın:

```bash
FLASHCODE_SERVER_PASSWORD=secret opencode web
```

Kullanıcı adı varsayılan olarak `opencode` şeklindedir ancak `FLASHCODE_SERVER_USERNAME` ile değiştirilebilir.
...
Sunucu ayarlarını `flashcode.json` yapılandırma dosyanızda da yapılandırabilirsiniz:
