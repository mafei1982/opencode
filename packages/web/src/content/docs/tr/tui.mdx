...
Bu, sunucu/çalışma zamanı davranışını yapılandıran `flashcode.json` dosyasından ayrıdır.
...
Özel bir TUI yapılandırma yolu yüklemek için `FLASHCODE_TUI_CONFIG` kullanın.
