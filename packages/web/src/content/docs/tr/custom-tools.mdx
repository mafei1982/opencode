...
- Yerel olarak bunları projenizin `.flashcode/tools/` dizinine yerleştirerek.
- Veya küresel olarak bunları `~/.config/flashcode/tools/` içine yerleştirerek.
...
```ts title=".flashcode/tools/database.ts" {1}
...
```ts title=".flashcode/tools/math.ts"
import { tool } from "@opencode-ai/plugin"
...
```ts title=".flashcode/tools/bash.ts"
import { tool } from "@opencode-ai/plugin"
...
```ts title=".flashcode/tools/project.ts" {8}
import { tool } from "@opencode-ai/plugin"
...
```python title=".flashcode/tools/add.py"
import sys
...
```ts title=".flashcode/tools/python-add.ts" {10}
import { tool } from "@opencode-ai/plugin"
...
export default tool({
  ...
  async execute(args, context) {
    const script = path.join(context.worktree, ".flashcode/tools/add.py")
