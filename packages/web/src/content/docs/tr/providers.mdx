...
`~/.local/share/flashcode/auth.json` içinde.
...
2. **Kimlik doğrulamayı yapılandırın**: Aşağıdaki yöntemlerden birini kullanın:
   ...
   Projeye özel veya kalıcı yapılandırma için `flashcode.json` kullanın:
...
`flashcode.json` dosyası. Ayrıca oturum paylaşımını devre dışı bırakmanız da önerilir.
...
`flashcode.json` üzerinden özelleştirin:
...
4. Proje dizininizde `flashcode.json` dosyanızı oluşturun veya güncelleyin:
