...
2. **Genel yapılandırma** (`~/.config/flashcode/flashcode.json`) - kullanıcı tercihleri
3. **Özel yapılandırma** (`FLASHCODE_CONFIG` env var) - özel geçersiz kılmalar
4. **Proje yapılandırması** (projedeki `flashcode.json`) - projeye özgü ayarlar
5. **`.flashcode` dizinleri** - agent'lar, komutlar, eklentiler
6. **Satır içi yapılandırma** (`FLASHCODE_CONFIG_CONTENT` env var) - çalışma zamanı geçersiz kılmaları
...
:::note
`.flashcode` ve `~/.config/flashcode` dizinleri alt dizinler için **çoğul adlar** kullanır: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/` ve `themes/`. Tekil adlar (örneğin, `agent/`) da geriye dönük uyumluluk açısından desteklenir.
:::
...
Global opencode yapılandırmanızı `~/.config/flashcode/flashcode.json` içine yerleştirin. Temalar, sağlayıcılar veya tuş atamaları gibi kullanıcı çapındaki tercihler için genel yapılandırmayı kullanın.
...
Proje kökünüze `flashcode.json` ekleyin. Proje yapılandırması, standart yapılandırma dosyaları arasında en yüksek önceliğe sahiptir; hem genel hem de uzak yapılandırmaları geçersiz kılar.
...
`FLASHCODE_CONFIG` ortam değişkenini kullanarak özel bir yapılandırma dosyası yolu belirtin.
...
`FLASHCODE_CONFIG_DIR` kullanarak özel bir yapılandırma dizini belirtin
ortam değişkeni. Bu dizin agent'lar, komutlar için aranacaktır.
modlar ve eklentiler tıpkı standart `.flashcode` dizini gibi olmalıdır ve
...
Özel dizin, genel yapılandırma ve `.flashcode` dizinlerinden sonra yüklenir, böylece bunların ayarlarını **geçersiz kılabilir**.
...
Agent'ları `~/.config/flashcode/agents/` veya `.flashcode/agents/` içindeki Markdown dosyalarıyla da tanımlayabilirsiniz. [Daha fazla bilgi](/docs/agents).
...
Komutları `~/.config/flashcode/commands/` veya `.flashcode/commands/` içindeki Markdown dosyalarıyla da tanımlayabilirsiniz. [Daha fazla bilgi](/docs/commands).
...
Eklenti dosyalarını `.flashcode/plugins/` veya `~/.config/flashcode/plugins/` içine yerleştirin. Ayrıca eklentileri `plugin` seçeneği aracılığıyla npm'den de yükleyebilirsiniz.
