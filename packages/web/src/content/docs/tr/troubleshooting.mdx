...
- **macOS/Linux**: `~/.config/flashcode/flashcode.jsonc` (veya `~/.config/flashcode/flashcode.json`)
- **macOS/Linux** (eski kurulumlar): `~/.local/share/flashcode/flashcode.jsonc`
- **Windows**: `WIN+R` tuslayip `%USERPROFILE%\.config\opencode\flashcode.jsonc` yapistirin
...
- **macOS**: Finder -> `Cmd+Shift+G` -> `~/.cache/flashcode`
- **Linux**: `~/.cache/flashcode` dizinini silin (veya `rm -rf ~/.cache/flashcode` calistirin)
...
`flashcode.json(c)` dosyanizda `server` bolumu varsa gecici olarak kaldirin ve desktop uygulamasini yeniden baslatin.
