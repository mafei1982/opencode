...
:::note
Bu araç yalnızca `FLASHCODE_EXPERIMENTAL_LSP_TOOL=true` (veya `FLASHCODE_EXPERIMENTAL=true`) olduğunda kullanılabilir.
:::
...
:::note
Bu araç yalnızca opencode provider kullanılırken veya `FLASHCODE_ENABLE_EXA` ortam değişkeni doğru değerlerden birine ayarlandığında (örneğin `true` ya da `1`) kullanılabilir.
