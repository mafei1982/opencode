...
2. **Globalna konfiguracija** (`~/.config/flashcode/flashcode.json`) - korisničke preferencije
3. **Prilagođena konfiguracija** (`FLASHCODE_CONFIG` env var) - prilagođena preinačenja
4. **Konfiguracija projekta** (`flashcode.json` u projektu) - postavke specifične za projekat
5. **`.flashcode` direktoriji** - agenti, komande, dodaci
6. **Inline konfiguracija** (`FLASHCODE_CONFIG_CONTENT` env var) - runtime preinačenja
...
:::note
Direktoriji `.flashcode` i `~/.config/flashcode` koriste **imena u množini** za poddirektorije: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/` i `themes/`. Pojedinačna imena (npr. `agent/`) su također podržana za kompatibilnost unatrag.
:::
...
Postavite svoju globalnu OpenCode konfiguraciju u `~/.config/flashcode/flashcode.json`. Koristite globalnu konfiguraciju za korisničke preferencije kao što su provajderi, modeli i dozvole.

Za postavke specifične za TUI, koristite `~/.config/flashcode/tui.json`.
...
Dodajte `flashcode.json` u korijen projekta. Konfiguracija projekta ima najveći prioritet među standardnim konfiguracijskim datotekama - ona nadjačava globalne i udaljene konfiguracije.
...
Navedite prilagođenu putanju konfiguracijske datoteke koristeći varijablu okruženja `FLASHCODE_CONFIG`.
...
Navedite prilagođeni konfiguracijski direktorij koristeći `FLASHCODE_CONFIG_DIR` varijablu okruženja. U ovom direktoriju će se tražiti agenti, komande, modovi i dodaci baš kao standardni `.flashcode` direktorij, i trebali bi pratiti istu strukturu.
...
Prilagođeni direktorij se učitava nakon direktorija globalne konfiguracije i `.flashcode`, tako da **može nadjačati** njihove postavke.
...
Koristite `FLASHCODE_TUI_CONFIG` da pokažete na prilagođeni TUI konfiguracijski fajl.

Stari `theme`, `keybinds`, i `tui` ključevi u `flashcode.json` su zastarjeli i automatski će se migrirati kada je to moguće.
...
Također možete definirati agente koristeći markdown datoteke u `~/.config/flashcode/agents/` ili `.flashcode/agents/`. [Saznajte više ovdje](/docs/agents).
...
Također možete definirati naredbe koristeći markdown fajlove u `~/.config/flashcode/commands/` ili `.flashcode/commands/`. [Saznajte više ovdje](/docs/commands).
...
Postavite datoteke dodataka u `.flashcode/plugins/` ili `~/.config/flashcode/plugins/`. Također možete učitati dodatke iz npm-a preko opcije `plugin`.
