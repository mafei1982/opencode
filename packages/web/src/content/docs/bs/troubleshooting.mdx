...
- **macOS/Linux**: `~/.config/flashcode/flashcode.jsonc` (ili `~/.config/flashcode/flashcode.json`)
- **macOS/Linux** (starije instalacije): `~/.local/share/flashcode/flashcode.jsonc`
- **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.config\opencode\flashcode.jsonc`
...
- **macOS**: Finder -> `Cmd+Shift+G` -> zalijepi `~/.cache/flashcode`
- **Linux**: obrišite `~/.cache/flashcode` (ili pokrenite `rm -rf ~/.cache/flashcode`)
...
Ako vaš `flashcode.json(c)` sadrži odjeljak `server`, privremeno ga uklonite i ponovo pokrenite desktop aplikaciju.
