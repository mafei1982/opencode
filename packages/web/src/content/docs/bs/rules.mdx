...
Mozete imati i globalna pravila u `~/.config/flashcode/AGENTS.md`. Ona se primjenjuju u svim opencode sesijama.
...
- **Globalna pravila**: `~/.claude/CLAUDE.md` (koristi se ako ne postoji `~/.config/flashcode/AGENTS.md`)
...
2. **Globalna datoteka** na `~/.config/flashcode/AGENTS.md`
3. **Claude Code datoteka** na `~/.claude/CLAUDE.md` (osim ako je iskljucena)

Prva pronadena datoteka pobjeduje u svakoj kategoriji. Na primjer, ako imate i `AGENTS.md` i `CLAUDE.md`, koristi se samo `AGENTS.md`. Isto tako, `~/.config/flashcode/AGENTS.md` ima prednost nad `~/.claude/CLAUDE.md`.

---

## Prilagođena uputstva

Mozete navesti prilagodene datoteke uputstava u `flashcode.json` ili globalnom `~/.config/flashcode/flashcode.json`. Tako vi i tim ponovo koristite postojeca pravila bez dupliranja u AGENTS.md.
...
### Korištenje flashcode.json

Preporuceni pristup je da koristite `instructions` polje u `flashcode.json`:
...
:::tip
Za monorepo projekte ili projekte sa zajednickim standardima, odrzivije je koristiti `flashcode.json` sa glob obrascima (npr. `packages/*/AGENTS.md`) nego rucna uputstva.
:::
