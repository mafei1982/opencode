...
u `~/.local/share/flashcode/auth.json`.
...
Za konfiguraciju specifičnu za projekat ili trajnu konfiguraciju, koristite `flashcode.json`:
...
`flashcode.json` fajl. Također se preporučuje da onemogućite dijeljenje sesije.
...
Prilagodite putem `flashcode.json`:
...
4. Kreirajte ili ažurirajte svoju `flashcode.json` datoteku u direktoriju projekta:
