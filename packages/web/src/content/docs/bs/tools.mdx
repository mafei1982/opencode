...
:::note
Ovaj alat je dostupan samo kada je `FLASHCODE_EXPERIMENTAL_LSP_TOOL=true` (ili `FLASHCODE_EXPERIMENTAL=true`).
:::
...
:::note
Ovaj alat je dostupan samo uz OpenCode provajdera ili kada je varijabla `FLASHCODE_ENABLE_EXA` postavljena na truthy vrijednost (npr. `true` ili `1`).
