...
в `~/.local/share/flashcode/auth.json`.
...
2. **Настройте аутентификацию** одним из следующих способов:
   ...
   Для конкретной или постоянной конфигурации проекта используйте `flashcode.json`:
...
`flashcode.json` файл. Также рекомендуется отключить совместное использование сеансов.
...
Настройте через `flashcode.json`:
...
4. Создайте или обновите файл `flashcode.json` в каталоге вашего проекта:
