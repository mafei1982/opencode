...
Når du tilføjer en udbyders API-nøgler med `/connect`-kommandoen, gemmes de i `~/.local/share/flashcode/auth.json`.
...
2. **Konfigurer godkendelse** ved at bruge en af følgende metoder:
   ...
   For projektspecifik eller vedvarende konfiguration, brug `flashcode.json`:
...
`flashcode.json` fil. Det anbefales også at deaktivere sessionsdeling.
...
4. Opret eller opdater `flashcode.json`-filen i projektkataloget:
