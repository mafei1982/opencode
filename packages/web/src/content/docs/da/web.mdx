...
:::caution
Hvis `FLASHCODE_SERVER_PASSWORD` ikke er indstillet, vil serveren være usikret. Dette er fint til lokal brug, men bør indstilles til netværksadgang.
:::
...
For at beskytte adgangen skal du angive en adgangskode ved hjælp af `FLASHCODE_SERVER_PASSWORD` miljøvariablen:

```bash
FLASHCODE_SERVER_PASSWORD=secret opencode web
```

Brugernavnet er som standard `opencode`, men kan ændres med `FLASHCODE_SERVER_USERNAME`.
...
Du kan også konfigurere serverindstillinger i din `flashcode.json`-konfigurationsfil:
