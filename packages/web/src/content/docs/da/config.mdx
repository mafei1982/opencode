...
2. **Global config** (`~/.config/flashcode/flashcode.json`) - brugerpræferencer
3. **Tilpasset konfiguration** (`FLASHCODE_CONFIG` env var) - tilpassede tilsidesættelser
4. **Project config** (`flashcode.json` i projekt) - projektspecifikke indstillinger
5. **`.flashcode` mapper** - agent, kommandoer, plugins
6. **Inline config** (`FLASHCODE_CONFIG_CONTENT` env var) - runtime tilsidesættelser
...
:::note
`.flashcode` og `~/.config/flashcode` mapperne bruger **flertalsnavne** for undermapper: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/` og `themes/`. Enkelte navne (f.eks. `agent/`) understøtter også bagudkompatibilitet.
:::
...
Placer din globale OpenCode-konfiguration i `~/.config/flashcode/flashcode.json`. Brug global konfiguration til brugerdækkende præferencer som temaer, udbydere eller nøglebindinger.
...
Tilføj `flashcode.json` i dit projektrod. Project config har den højeste forrang blandt standard config-filer - den tilsidesætter både globale og eksterne config.
...
Angiv en brugerdefineret konfigurationsfilsti ved hjælp af miljøvariablen `FLASHCODE_CONFIG`.
...
Angiv en brugerdefineret konfigurationsmappe ved hjælp af `FLASHCODE_CONFIG_DIR`
miljøvariabel. Dette kort vil blive søgt efter agenter, kommandoer,
modes og plugins ligesom standard `.flashcode` mappen, og bør
...
Den brugerdefinerede mappe indlæses efter den globale konfig og `.flashcode` mapper, så den **kan tilsidesætte** deres indstillinger.
...
Du kan også definere agenter ved at bruge markdown-filer i `~/.config/flashcode/agents/` eller `.flashcode/agents/`. [Learn more here](/docs/agents).
...
Du kan også definere kommandoer ved hjælp af markdown-filer i `~/.config/flashcode/commands/` eller `.flashcode/commands/`. [Learn more here](/docs/commands).
...
Placer plugin-filer i `.flashcode/plugins/` eller `~/.config/flashcode/plugins/`. Du kan også indlæse plugins fra npm gennem indstillingen `plugin`.
