...
:::note
Dette verktøyet er bare tilgængelig når `FLASHCODE_EXPERIMENTAL_LSP_TOOL=true` (eller `FLASHCODE_EXPERIMENTAL=true`).
:::
...
:::note
Dette verktøyet er bare tilgængelig når du bruger OpenCode-leverandøren eller når miljøvariabelen `FLASHCODE_ENABLE_EXA` er satt til en sannverdi (f.eks. `true` eller `1`).
