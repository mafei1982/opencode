...
- **macOS/Linux**: `~/.config/flashcode/flashcode.jsonc` (eller `~/.config/flashcode/flashcode.json`)
- **macOS/Linux** (ældre installationer): `~/.local/share/flashcode/flashcode.jsonc`
- **Windows**: Tryk `WIN+R` og indsæt `%USERPROFILE%\.config\opencode\flashcode.jsonc`
...
- **macOS**: Finder -> `Cmd+Shift+G` -> indsæt `~/.cache/flashcode`
- **Linux**: slet `~/.cache/flashcode` (eller kør `rm -rf ~/.cache/flashcode`)
...
Hvis `flashcode.json(c)` indeholder en `server`-del, fjern den midlertidigt og start skrivebordsappen på ny.
