...
:::caution
Lorsque vous utilisez `--hostname 0.0.0.0`, définissez `FLASHCODE_SERVER_PASSWORD` pour sécuriser le serveur.

```bash
FLASHCODE_SERVER_PASSWORD=your-password opencode serve --hostname 0.0.0.0
```
...
- Votre configuration OpenCode et vos sessions sont stockées dans l'environnement WSL à `~/.local/share/flashcode/`
