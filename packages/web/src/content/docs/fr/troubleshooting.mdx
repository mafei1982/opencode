...
- **macOS/Linux** : `~/.config/flashcode/flashcode.jsonc` (ou `~/.config/flashcode/flashcode.json`)
- **macOS/Linux** (anciennes installations) : `~/.local/share/flashcode/flashcode.jsonc`
- **Windows** : appuyez sur `WIN+R` et collez `%USERPROFILE%\.config\opencode\flashcode.jsonc`
...
- **macOS** : Finder -> `Cmd+Shift+G` -> coller `~/.cache/flashcode`
- **Linux** : supprimez `~/.cache/flashcode` (ou exécutez `rm -rf ~/.cache/flashcode`)
...
Si votre `flashcode.json(c)` contient une section `server`, supprimez-la temporairement et redémarrez l'application de bureau.
