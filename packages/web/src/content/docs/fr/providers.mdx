...
en `~/.local/share/flashcode/auth.json`.
...
Pour une configuration spécifique au projet ou persistante, utilisez `flashcode.json` :
...
Fichier `flashcode.json`. Il est également recommandé de désactiver le partage de session.
...
Personnalisez via `flashcode.json` :
...
4. Créez ou mettez à jour votre fichier `flashcode.json` dans le répertoire de votre projet :
