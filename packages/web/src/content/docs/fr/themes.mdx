...
2. **Répertoire de configuration utilisateur** - Défini dans `~/.config/flashcode/themes/*.json` ou `$XDG_CONFIG_HOME/opencode/themes/*.json`
3. **Répertoire racine du projet** - Défini dans `<project-root>/.flashcode/themes/*.json`
4. **Répertoire de travail actuel** - Défini dans `./.flashcode/themes/*.json`
