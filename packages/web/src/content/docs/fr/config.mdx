...
2. **Configuration globale** (`~/.config/flashcode/flashcode.json`) - préférences utilisateur
3. **Configuration personnalisée** (`FLASHCODE_CONFIG` env var) - remplacements personnalisés
4. **Configuration du projet** (`flashcode.json` dans le projet) - paramètres spécifiques au projet
5. **Répertoires `.flashcode`** - agents, commandes, plugins
6. **Configuration en ligne** (`FLASHCODE_CONFIG_CONTENT` env var) - remplacements d'exécution
...
:::note
Les répertoires `.flashcode` et `~/.config/flashcode` utilisent des **noms au pluriel** pour les sous-répertoires : `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/` et `themes/`. Les noms singuliers (par exemple, `agent/`) sont également pris en charge pour une compatibilité ascendante.
:::
...
Placez votre configuration globale OpenCode dans `~/.config/flashcode/flashcode.json`. Utilisez la configuration globale pour les préférences de l'utilisateur telles que les fournisseurs, les modèles et les autorisations.

Pour les paramètres spécifiques à TUI, utilisez `~/.config/flashcode/tui.json`.
...
Ajoutez `flashcode.json` à la racine de votre projet. La configuration du projet a la priorité la plus élevée parmi les fichiers de configuration standard : elle remplace les configurations globales et distantes.
...
Spécifiez un chemin de fichier de configuration personnalisé à l'aide de la variable d'environnement `FLASHCODE_CONFIG`.
...
Spécifiez un répertoire de configuration personnalisé à l'aide de `FLASHCODE_CONFIG_DIR` variable d'environnement. Ce répertoire sera recherché pour les agents, les commandes, modes et plugins tout comme le répertoire standard `.flashcode`, et devrait suivre la même structure.
...
Le répertoire personnalisé est chargé après les répertoires de configuration globale et `.flashcode`, il **peut donc remplacer** leurs paramètres.
...
Utilisez `FLASHCODE_TUI_CONFIG` pour pointer vers un fichier de configuration TUI personnalisé.

Les anciennes clés `theme`, `keybinds` et `tui` dans `flashcode.json` sont obsolètes et migrées automatiquement lorsque cela est possible.
...
Vous pouvez également définir des agents à l'aide de fichiers markdown dans `~/.config/flashcode/agents/` ou `.flashcode/agents/`. [En savoir plus ici](/docs/agents).
...
Vous pouvez également définir des commandes à l'aide de fichiers markdown dans `~/.config/flashcode/commands/` ou `.flashcode/commands/`. [En savoir plus ici](/docs/commands).
...
Placez les fichiers du plugin dans `.flashcode/plugins/` ou `~/.config/flashcode/plugins/`. Vous pouvez également charger des plugins depuis npm via l'option `plugin`.
