...
:::note
Cet outil n'est disponible que lorsque `FLASHCODE_EXPERIMENTAL_LSP_TOOL=true` (ou `FLASHCODE_EXPERIMENTAL=true`).
:::
...
:::note
Cet outil n'est disponible que lors de l'utilisation du fournisseur OpenCode ou lorsque la variable d'environnement `FLASHCODE_ENABLE_EXA` est définie sur une valeur véridique (par exemple, `true` ou `1`).
