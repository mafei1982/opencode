...
Vous pouvez également avoir des règles globales dans un fichier `~/.config/flashcode/AGENTS.md`. Ceci est appliqué à toutes les sessions opencode.
...
- **Règles globales** : `~/.claude/CLAUDE.md` (utilisé si aucun `~/.config/flashcode/AGENTS.md` n'existe)
...
```bash
export FLASHCODE_DISABLE_CLAUDE_CODE=1        # Disable all .claude support
export FLASHCODE_DISABLE_CLAUDE_CODE_PROMPT=1 # Disable only ~/.claude/CLAUDE.md
export FLASHCODE_DISABLE_CLAUDE_CODE_SKILLS=1 # Disable only .claude/skills
```
...
2. **Fichier global** à `~/.config/flashcode/AGENTS.md`
3. **Fichier Claude Code** à `~/.claude/CLAUDE.md` (sauf si désactivé)

Le premier fichier correspondant gagne dans chaque catégorie. Par exemple, si vous possédez à la fois `AGENTS.md` et `CLAUDE.md`, seul `AGENTS.md` est utilisé. De même, `~/.config/flashcode/AGENTS.md` a priorité sur `~/.claude/CLAUDE.md`.

---

## Instructions personnalisées

Vous pouvez spécifier des fichiers d'instructions personnalisés dans votre `flashcode.json` ou dans le `~/.config/flashcode/flashcode.json` global. Cela vous permet, à vous et à votre équipe, de réutiliser les règles existantes plutôt que d'avoir à les dupliquer dans AGENTS.md.
...
### Utilisation de flashcode.json

L'approche recommandée consiste à utiliser le champ `instructions` dans `flashcode.json` :
...
:::tip
Pour les monorepos ou les projets avec des normes partagées, l'utilisation de `flashcode.json` avec des modèles globaux (comme `packages/*/AGENTS.md`) est plus maintenable que les instructions manuelles.
:::
