...
- **macOS/Linux**: `~/.config/flashcode/flashcode.jsonc` (or `~/.config/flashcode/flashcode.json`)
- **macOS/Linux** (older installs): `~/.local/share/flashcode/flashcode.jsonc`
- **Windows**: Press `WIN+R` and paste `%USERPROFILE%\.config\opencode\flashcode.jsonc`
...
- **Linux**: delete `~/.cache/flashcode` (or run `rm -rf ~/.cache/flashcode`)
...
If your `flashcode.json(c)` contains a `server` section, temporarily remove it and restart the desktop app.
