...
export function SessionSidePanel(props: {
  ...
  const handleRename = async (node: { path: string; name: string }, newName: string) => {
    ...
    const target = parent ? `${parent}${node.path[index]}${newName}` : newName
    try {
      await file.rename(node.path, target)
    } catch (error) {
      showToast({
        variant: "error",
        title: "Rename failed",
  ...
  const handleCopy = async (node: { path: string; name: string }) => {
    const extension = node.name.lastIndexOf(".")
    const name = extension > 0 ? node.name.slice(0, extension) : node.name
    const suffix = extension > 0 ? node.name.slice(extension) : ""
    const index = Math.max(node.path.lastIndexOf("/"), node.path.lastIndexOf("\\"))
    const parent = index === -1 ? "" : node.path.slice(0, index)
    const target = parent ? `${parent}${node.path[index]}${name} - Copy${suffix}` : `${name} - Copy${suffix}`
    try {
      await file.copy(node.path, target)
    } catch (error) {
      showToast({
        variant: "error",
        title: "Copy failed",
  ...
  const handleReveal = (node: { path: string }) => {
    if (!projectDir() || !platform.openPath) return
    const index = Math.max(node.path.lastIndexOf("/"), node.path.lastIndexOf("\\"))
    void platform.openPath(index === -1 ? projectDir() : `${projectDir()}/${node.path.slice(0, index)}`)
  }
                      ...
                      <Show when={activeTab() === "empty"}>
                        <div class="relative pt-2 flex-1 min-h-0 overflow-hidden">
                          <div class="h-full px-6 pb-42 -mt-4 flex flex-col items-center justify-center text-center gap-6">
                            <Logo class="h-auto w-36 max-w-full opacity-10" />
